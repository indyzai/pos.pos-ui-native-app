...
The merge algorithm is already shared in `@openpos/core`; the remaining duplication is the orchestration state machine around backend IO and UI notification.

## Decision

Plan a follow-up refactor that moves the platform-independent sync orchestration state machine into `@openpos/core`.
