...
OpenPOS already ships file-based BYOS sync and other backends using full-snapshot merge. The current sync model is revision-aware and deterministic:
...
OpenPOS keeps snapshot merge and does not add a delta log at this time.
...
- We should revisit this decision only if one or more of these thresholds are crossed:
  ...
  - OpenPOS needs real-time multi-device streaming
- If that revisit happens, the first design to evaluate is an append-only `openpos-delta.jsonl` alongside `openpos-snapshot.json`, built on top of the existing `rev` and `revBy` metadata, keeping the current conflict resolution rules, compacting by the highest revision per entity id, and tracking watermarks per device.
