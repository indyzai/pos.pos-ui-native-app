...
OpenPOS persists structured data in SQLite on desktop and mobile, while sync backends exchange a JSON snapshot (`data.json`) plus attachments.
...
OpenPOS keeps SQLite and JSON, but with an asymmetric contract:
