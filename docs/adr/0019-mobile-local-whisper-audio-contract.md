...
OpenPOS supports local Whisper transcription on mobile through `whisper.rn`. Two bug histories exposed the same underlying contract problem:
...
OpenPOS treats mobile local ASR as an audio-contract boundary, not a model-loading detail.
...
- Model cache repair is allowed only for OpenPOS-owned local Whisper cache paths. It must not be generalized to user-selected files, attachments, or synced data.
