...
This is the second release candidate for OpenPOS 1.1.0. Use it for tester channels only. Prepare final stable `1.1.0` notes separately when the stable release is cut.
...
- Updated `openpos-mcp` (1.1.1) so global npm installs can use current prebuilt `better-sqlite3` binaries on Node 22+ (Node 20 still works but needs build tools).
...
- fix(mcp): `npm i -g openpos-mcp` no longer needs a C++ compiler on Windows with Node 22+ (openpos-mcp 1.1.1).
