...
- OpenPOS 1.0.0 publishes the local MCP automation helper as the public npm package `openpos-mcp` and MCP Registry server `io.github.dongdongbh/openpos`, with safer nullable-field handling, idempotent publish checks, and zero-warning lint coverage.
...
- Published the MCP server as `openpos-mcp` and added MCP Registry metadata.
