type BillingSnapshot = {
    ...
    session: any;
    queue: { id: string; [key: string]: any }[];
    updated?: string;
};
...
type ScopedSale = BillingSnapshot["queue"][number] & {
    storageId: string;
    scope: string;
};
type BillingMetadata = Pick<BillingSnapshot, "session" | "updated"> & {
    scope: string;
};
    ...
    if (!databasePromise) {
        databasePromise = new Promise<IDBDatabase>((resolve, reject) => {
            ...
            request.onerror = () =>
                reject(
                    request.error ??
                        new Error("Unable to open browser storage."),
                );
            request.onblocked = () =>
                reject(
                    new Error("Close other POS tabs and retry local storage."),
                );
                ...
                if (database.objectStoreNames.contains(legacyStore)) {
                    transaction
                        ...
                        for (const product of snapshot.products ?? [])
                            transaction.objectStore(webStores.products).put({
                                ...product,
                                scope,
                                storageId: `${scope}:product:${product.id}`,
                            });
                        for (const sale of snapshot.queue ?? [])
                            transaction.objectStore(webStores.sales).put({
                                ...sale,
                                scope,
                                storageId: `${scope}:sale:${sale.id}`,
                            });
...
function readByScope<T>(store: IDBObjectStore, scope: string): Promise<T[]> {
    return new Promise((resolve, reject) => {
        ...
        request.onerror = () =>
            reject(
                request.error ?? new Error("Unable to read browser storage."),
            );
        request.onsuccess = () => resolve(request.result as T[]);
    });
}

export async function readWebBillingSnapshot<T = BillingSnapshot>(
    scope: string,
): Promise<T | undefined> {
    ...
    const [productRows, saleRows, metadata] = await Promise.all([
        readByScope<ScopedProduct>(
            transaction.objectStore(webStores.products),
            scope,
        ),
        readByScope<ScopedSale>(
            transaction.objectStore(webStores.sales),
            scope,
        ),
        new Promise<BillingMetadata | undefined>((resolve, reject) => {
            const request = transaction
                .objectStore(webStores.metadata)
                .get(scope);
            request.onerror = () =>
                reject(
                    request.error ??
                        new Error("Unable to read billing metadata."),
                );
            request.onsuccess = () =>
                resolve(request.result as BillingMetadata | undefined);
        }),
    ]);
    if (!metadata && !productRows.length && !saleRows.length) return undefined;
    return {
        products: productRows.map(
            ({ storageId: _storageId, scope: _scope, ...product }) => product,
        ),
        queue: saleRows.map(
            ({ storageId: _storageId, scope: _scope, ...sale }) => sale,
        ),
...
export async function writeWebBillingSnapshot(
    scope: string,
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        transaction.onerror = () =>
            reject(
                transaction.error ??
                    new Error("Unable to write browser storage."),
            );
        transaction.onabort = () =>
            reject(
                transaction.error ??
                    new Error("Browser storage write was aborted."),
            );
            ...
            keys.onsuccess = () => {
                ...
                for (const record of records)
                    store.put({
                        ...record,
                        scope,
                        storageId: `${scope}:${kind}:${record.id}`,
                    });
            };
        };
...
export async function readWebSyncJobs(scope: string): Promise<WebSyncJob[]> {
    ...
    return readByScope<WebSyncJob>(
        transaction.objectStore(webStores.syncJobs),
        scope,
    );
}
...
export async function writeWebSyncJob(job: WebSyncJob): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        transaction.onerror = () =>
            reject(transaction.error ?? new Error("Unable to save sync job."));
        transaction.oncomplete = () => resolve();
...
export async function writeWebHeldOrder<T extends { id: string }>(
    scope: string,
    order: T,
): Promise<void> {
    const database = await openDatabase();
    await new Promise<void>((resolve, reject) => {
        const transaction = database.transaction(
            webStores.heldOrders,
            "readwrite",
        );
        transaction.onerror = () =>
            reject(
                transaction.error ?? new Error("Unable to save held order."),
            );
        transaction.oncomplete = () => resolve();
...
export async function deleteWebHeldOrder(
    scope: string,
    id: string,
): Promise<void> {
    const database = await openDatabase();
    await new Promise<void>((resolve, reject) => {
        ...
        transaction.onerror = () =>
            reject(
                transaction.error ?? new Error("Unable to delete held order."),
            );
        transaction.oncomplete = () => resolve();
        transaction
            .objectStore(webStores.heldOrders)
            .delete(`${scope}:held-order:${id}`);
    });
}
...
export async function replaceWebScopedRecords<T extends { id: string }>(
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        transaction.onerror = () =>
            reject(
                transaction.error ??
                    new Error("Unable to save reference data."),
            );
        transaction.oncomplete = () => resolve();
        const store = transaction.objectStore(name);
        const keys = store.index("scope").getAllKeys(scope);
        keys.onsuccess = () => {
            ...
            for (const record of records)
                store.put({
                    ...record,
                    scope,
                    storageId: `${scope}:${name}:${record.id}`,
                });
        };
    });
}
...
export async function readWebPrintJobs<T>(scope: string): Promise<T[]> {
    ...
    return readByScope<T>(
        database
            .transaction(webStores.printJobs, "readonly")
            .objectStore(webStores.printJobs),
        scope,
    );
}

export async function writeWebPrintJob<T extends { id: string }>(
    scope: string,
    job: T,
): Promise<void> {
    const database = await openDatabase();
    await new Promise<void>((resolve, reject) => {
        ...
        transaction.onerror = () =>
            reject(transaction.error ?? new Error("Unable to save print job."));
        transaction.oncomplete = () => resolve();
        transaction
            .objectStore(webStores.printJobs)
...
export async function clearWebLocalData(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        transaction.onerror = () =>
            reject(
                transaction.error ??
                    new Error("Unable to clear browser storage."),
            );
        transaction.onabort = () =>
            reject(
                transaction.error ??
                    new Error("Clearing browser storage was aborted."),
            );
        transaction.oncomplete = () => resolve();
