...
import { PosIndexedDb } from "./client";
...
> implements CollectionRepository<T> {
    ...
    async list(query: LocalQuery = {}) {
        let rows = await this.table()
            ...
            .toArray();
        if (query.storeId)
            rows = rows.filter((row) => row.storeId === query.storeId);
        if (query.syncStatus)
            rows = rows.filter((row) => row.syncStatus === query.syncStatus);
...
export class IndexedDbLocalDatabase implements LocalDatabase {
    ...
    collection<T extends LocalRecord = LocalRecord>(
        ...
        if (!repository) {
            repository = new IndexedDbCollection(
                this.database,
                name,
                this.scope,
            );
            ...
            async () => {
                for (const name of collectionNames) {
                    ...
                    const rows = await table
                        .where("tenantId")
                        .equals(this.scope.tenantId)
                        .toArray();
                    const removable = rows
                        .filter(
                            (row) =>
                                row.syncStatus === "SYNCED" &&
                                (!allowed.has(name) || row.scope !== scopeKey),
                        )
                        .map((row) => row.id);
    ...
    async clear() {
        await this.database.transaction(
            ...
            async () => {
                await Promise.all(
                    this.database.tables.map((table) => table.clear()),
                );
            },
        );
    }
