...
export function useLocalRecord<T extends LocalRecord = LocalRecord>(
    name: CollectionName,
    id?: string,
) {
    const { database, status } = useLocalDatabase();
...
export const useLocalProducts = <T extends LocalRecord = LocalRecord>(
    query?: LocalQuery,
) => useLocalCollection<T>("products", query);
export const useLocalCustomers = <T extends LocalRecord = LocalRecord>(
    query?: LocalQuery,
) => useLocalCollection<T>("customers", query);
export const useLocalSales = <T extends LocalRecord = LocalRecord>(
    query?: LocalQuery,
) => useLocalCollection<T>("sales", query);
export const useLocalPayments = <T extends LocalRecord = LocalRecord>(
    query?: LocalQuery,
) => useLocalCollection<T>("payments", query);
export const useLocalStockBalances = <T extends LocalRecord = LocalRecord>(
    query?: LocalQuery,
