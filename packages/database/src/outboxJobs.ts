...
export type OutboxJobOperation =
    "CREATE_PRODUCT" | "UPDATE_PRODUCT" | "SAVE_STOCK_DRAFT" | "UPDATE_STOCK";
...
function toRecord(
    db: LocalDatabase,
    job: OutboxJob,
): LocalRecord<InventoryOutboxPayload> {
...
export async function saveOutboxJob(job: OutboxJob): Promise<void> {
    ...
    await db
        .collection<LocalRecord<InventoryOutboxPayload>>("sync_outbox")
        .put(toRecord(db, job));
}
