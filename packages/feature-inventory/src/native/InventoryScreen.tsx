...
export function InventoryScreen({
    ...
    const data = useInventory(api);
    const { setFeatureLoading, setFeatureRefresh, setRefreshJob } =
        useAppHeader();
    ...
    return (
        <View style={[styles.screen, { backgroundColor: c.background }]}>
            ...
            >
                ...
                {activeSyncJob ? <SyncJobStatus job={activeSyncJob} /> : null}
            ...
            <BarcodeScannerModal
                ...
                onScan={(value) => {
                    ...
                    if (match && canEditInventory) setEditingProduct(match);
                        ...
                        showSnackbar(
                            "Barcode not found",
                            "No local inventory item matches this barcode.",
                        );
                }}
            />
...
function SyncJobStatus({
    ...
    job: ReturnType<typeof useInventory>["jobs"][number];
}) {
    if (job.status === "COMPLETED") return null;
    const { themeColors: c } = useAppTheme();
    const failed = job.status === "FAILED";
    const color = failed ? c.error : "#D97706";
    const Icon = failed ? AlertCircle : Clock3;
    const operation =
    ...
    return (
        <View
            style={[
                styles.job,
                { backgroundColor: `${color}12`, borderColor: `${color}45` },
            ]}
        >
            ...
            <View style={styles.jobCopy}>
                ...
                <Text style={[styles.jobId, { color: c.textSecondary }]}>
                    Queue ID {job.id}
                </Text>
                {job.errorMessage ? (
                    <Text style={[styles.jobError, { color: c.error }]}>
                        {job.errorMessage}
                    </Text>
                ) : null}
            </View>
