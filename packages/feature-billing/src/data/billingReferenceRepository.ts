...
import {
    ...
    serviceUsers,
    taxRates,
} from "@indyzai/pos-database/schema";
import {
    readWebScopedRecords,
    replaceWebScopedRecords,
    webStores,
} from "@indyzai/pos-database/web-client";
import type {
...
type ReferenceRecord =
    Customer | BillingPaymentMethod | ServiceUser | BillingTaxRate;
type ReferenceTable =
    | typeof customers
...
async function readCollection<T extends ReferenceRecord>(
    ...
    if (!hasNativeDatabase) {
        const rows = await readWebScopedRecords<
            T & { storageId: string; scope: string }
        >(webStore, scope);
        return rows.map(
            ({ storageId: _storageId, scope: _scope, ...record }) =>
...
async function replaceCollection<T extends ReferenceRecord>(
    ...
    if (!hasNativeDatabase)
        return replaceWebScopedRecords(webStore, scope, records);
    initializeDatabase();
...
export const billingReferenceRepository = {
    readCustomers: (scope: string) =>
        readCollection<Customer>(scope, customers, webStores.customers),
    ...
    readPaymentMethods: (scope: string) =>
        readCollection<BillingPaymentMethod>(
            scope,
            paymentMethods,
            webStores.paymentMethods,
        ),
    ...
    readServiceUsers: (scope: string) =>
        readCollection<ServiceUser>(
            scope,
            serviceUsers,
            webStores.serviceUsers,
        ),
    replaceServiceUsers: (scope: string, records: ServiceUser[]) =>
        replaceCollection(scope, records, serviceUsers, webStores.serviceUsers),
    readTaxRates: (scope: string) =>
        readCollection<BillingTaxRate>(scope, taxRates, webStores.taxRates),
