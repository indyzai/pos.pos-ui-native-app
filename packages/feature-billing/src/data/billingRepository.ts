...
import {
    readWebBillingSnapshot,
    writeWebBillingSnapshot,
...
const productId = (scope: string, remoteId: string) =>
    `${scope}:product:${remoteId}`;
const saleId = (scope: string, offlineId: string) =>
    `${scope}:sale:${offlineId}`;
...
const productFromRow = (product: typeof products.$inferSelect): Product => ({
    ...
    categoryType:
        (product.categoryType as Product["categoryType"]) ?? undefined,
    price: product.price,
    stock: product.stock,
...
export async function readBillingSnapshot(
    ...
    if (!hasNativeDatabase) {
        return (
            (await readWebBillingSnapshot(scope)) ?? {
                products: [],
                session: null,
                queue: [],
            }
    ...
    const productRows = nativeDatabase
        .select()
        .from(products)
        .where(eq(products.scope, scope))
        .all();
