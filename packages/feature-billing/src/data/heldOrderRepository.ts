...
import type { HeldOrder } from "@indyzai/feature-billing/types/billing";
...
export async function listHeldOrders(scope: string): Promise<HeldOrder[]> {
    if (!hasNativeDatabase) {
        const records = await readWebHeldOrders<
            HeldOrder & { storageId: string; scope: string }
        >(scope);
        return records
...
export async function removeHeldOrder(
    scope: string,
    id: string,
): Promise<void> {
