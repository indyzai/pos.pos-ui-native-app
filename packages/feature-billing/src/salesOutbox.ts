...
export function createPendingSale({
    ...
    if (!session && requireOpenSession)
        throw new Error("Open a counter session before creating a bill.");
    if (
        !items.length ||
        items.some(
            (item) =>
    ...
    const appliedOrderDiscount = Math.min(
        afterItemDiscount,
        Math.max(0, orderDiscount),
    );
    const orderDiscountRatio =
        afterItemDiscount > 0 ? appliedOrderDiscount / afterItemDiscount : 0;
    const lines = items.map((item) => {
        ...
        const discountedLine =
            Math.max(0, lineTotal - lineDiscount) * (1 - orderDiscountRatio);
    ...
    const checkoutPayment: CheckoutPayment =
        typeof payment === "string" ? { method: payment } : payment;
    const id = Crypto.randomUUID();
    const createdAt = new Date().toISOString();
    const receiptNumber = provisionalReceiptNumber(
        id,
        createdAt,
        invoicePrefix,
        session?.counterId,
    ...
    const primaryPayment = Math.max(
        0,
        Math.round((totals.total - scrapValue) * 100) / 100,
    );
    const payments = [
        ...(scrapValue > 0
            ? [
                  {
                      paymentMethod: "SCRAP",
                      amountPaid: scrapValue,
                      referenceNumber: checkoutPayment.scrap!.id,
                  },
              ]
            : []),
        ...(primaryPayment > 0
    ...
    const input: SaleInput = {
        ...
        details: {
            ...(checkoutPayment.tenderedAmount !== undefined
                ...
                : {}),
            ...(checkoutPayment.changeDue !== undefined
                ? { changeDue: checkoutPayment.changeDue }
                : {}),
            ...
            ...(checkoutPayment.scrap
                ? { scrapExchange: checkoutPayment.scrap }
                : {}),
            ...orderContext,
            ...(orderContext?.autoCreateWaybill && customer?.address
                ? {
                      waybillDraft: {
                          ...
                          sellerAddress:
                              orderContext.waybillSeller?.address || "",
...
export async function syncPendingSales(
    ...
    while (queue.length) {
        ...
        try {
            ...
            if (
                partyId != null &&
                (!Number.isSafeInteger(Number(partyId)) || Number(partyId) <= 0)
            ) {
                throw new Error(
                    "This bill is waiting for its customer to synchronize.",
                );
            }
            const data = await request<{
                saveBill: { id: string; billId: string };
            }>(createBillMutation, {
                newBillData: {
                    ...sale.input,
                    ...(partyId != null ? { partyId: Number(partyId) } : {}),
                },
            });
            if (!data.saveBill?.id)
