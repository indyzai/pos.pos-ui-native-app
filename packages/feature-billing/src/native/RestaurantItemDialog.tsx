...
export function RestaurantItemDialog({
    ...
    const modifiers = useMemo(
        () => restaurantModifiers(product?.details),
        [product?.details],
    );
    useEffect(() => {
        if (product) {
    ...
    const chosen = useMemo(
        () => modifiers.filter((item) => selected.includes(item.id)),
        [selected],
    );
    ...
    const toggle = (id: string) => {
        ...
        if (modifier)
            setSelected((current) =>
                toggleRestaurantModifier(current, modifier, modifiers),
            );
    };
    const customization: CartCustomization = {
        ...
        metadata: {
            modifiers: chosen.map(({ id, label, group, price }) => ({
                id,
                label,
                group,
                price,
                ...
                <View style={[s.sheet, { backgroundColor: c.surface }]}>
                    ...
                    >
                        <Text style={s.addText}>
                            Add {quantity} ·{" "}
                            {formatCurrency(
                                ((product?.price || 0) + adjustment) * quantity,
                                currencyCode,
                            )}
                        </Text>
                    </AppPressable>
...
const s = StyleSheet.create({
    ...
    backdrop: {
        ...
        backgroundColor: "rgba(8,12,22,.55)",
    },
    sheet: {
        maxHeight: "82%",
