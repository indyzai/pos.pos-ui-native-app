...
export function ServiceConfigurationDialog({
    ...
    const updatePart = (part: Product, change: number) =>
        setParts((current) => {
            const quantity =
                (current.find((item) => item.product.id === part.id)
                    ?.quantity || 0) + change;
            if (quantity <= 0)
                return current.filter((item) => item.product.id !== part.id);
            if (quantity > part.stock) return current;
            return current.some((item) => item.product.id === part.id)
                ? current.map((item) =>
                      item.product.id === part.id
                          ? { ...item, quantity }
                          : item,
                  )
    ...
    return (
        ...
        >
            <AppKeyboardSafeView style={s.overlay}>
                <AppPressable
                    accessibilityLabel="Close service configuration"
                    onPress={onClose}
                    style={s.backdrop}
                />
                <SafeAreaView
                ...
                >
                    ...
                    >
                        ...
                        {availableParts.map((item) => (
                            <AppPressable
                                key={item.id}
                                onPress={() => updatePart(item, 1)}
                                style={s.searchResult}
                            >
                                <Text
                                    numberOfLines={1}
                                    style={[s.partName, { color: c.text }]}
                                >
                                    {item.name}
                                </Text>
                                <Text
                                    style={[s.partPrice, { color: c.primary }]}
                                >
                        ...
                        {parts.map((item) => (
                            ...
                            >
                                ...
                                <AppPressable
                                    onPress={() => updatePart(item.product, -1)}
                                    style={s.qtyButton}
                                >
                                    <Minus size={14} color={c.text} />
                                </AppPressable>
                                <Text style={[s.quantity, { color: c.text }]}>
                                    {item.quantity}
                                </Text>
                                <AppPressable
                                    onPress={() => updatePart(item.product, 1)}
                    ...
                    <View
                        style={[s.footer, { borderTopColor: c.outlineMuted }]}
                    >
                        <View>
                            <Text
                                style={[
                        ...
                        <AppPressable
                            onPress={confirm}
                            style={[s.confirm, { backgroundColor: c.primary }]}
                        >
...
function Choice({
    ...
    return (
        ...
        >
            <Text
                style={[s.choiceText, { color: active ? "#FFFFFF" : c.text }]}
            >
                {label}
            </Text>
        </AppPressable>
