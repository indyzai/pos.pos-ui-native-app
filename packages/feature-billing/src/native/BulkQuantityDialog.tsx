...
import {
    resolveWholesaleTier,
    wholesaleTiers,
} from "@indyzai/feature-billing/domain/wholesalePricing";
...
export function BulkQuantityDialog({
    ...
    const selectedTier = product
        ? resolveWholesaleTier(product, amount)
        : undefined;
    return (
        <Modal
            transparent
                    ...
                    <TextInput
                        ...
                        style={[
                            s.input,
                            {
                                ...
                                borderColor:
                                    valid || !quantity ? c.outline : c.error,
                            },
                        ]}
                    />
                    {quantity && !valid ? (
                        <Text style={[s.error, { color: c.error }]}>
                            Enter between 1 and {product?.stock || 0}.
                        </Text>
                    ) : null}
                    <AppPressable
                        disabled={!valid}
