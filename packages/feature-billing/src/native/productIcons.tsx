...
export const productIconRegistry = {
    ...
    string,
    { label: string; icon: LucideIcon; color: string; fill?: string }
>;
...
export function ProductIcon({
    ...
    const entry =
        productIconRegistry[iconKey as ProductIconKey] ||
        productIconRegistry.package;
    const Icon = entry.icon;
    return (
        <Icon
            size={size}
            color={entry.color}
            fill={"fill" in entry ? entry.fill : "none"}
            strokeWidth={1.8}
        />
    );
}
