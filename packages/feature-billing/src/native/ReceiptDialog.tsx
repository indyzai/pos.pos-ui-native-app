...
/** Receipt content shared by operational and administrative order views. */
...
export function ReceiptDialog({
    ...
    const money = (value: number) =>
        formatCurrency(value, receipt.currencyCode);
    const share = () =>
        Share.share({
            message: [
    ...
    return (
        ...
        >
            <View style={s.overlay}>
                ...
                <SafeAreaView
                    edges={["bottom"]}
                    style={[s.sheet, { backgroundColor: c.surface }]}
                >
                    <View style={s.success}>
                        ...
                        <Text style={[s.receiptId, { color: c.textSecondary }]}>
                            Receipt {receipt.id}
                        </Text>
                    </View>
                    <ScrollView contentContainerStyle={s.body}>
                        ...
                        >
                            <Text style={[s.business, { color: c.text }]}>
                                {receipt.businessName}
                            </Text>
                            ...
                            {receipt.customer && (
                                <Text
                                    style={[s.meta, { color: c.textSecondary }]}
                                >
                                    Customer · {receipt.customer.name}
                                </Text>
                            )}
                            ...
                            <View
                                style={[
                                    s.divider,
                                    { backgroundColor: c.outlineMuted },
                                ]}
                            />
                            {receipt.items.map((item) => (
                                ...
                                >
                                    <Text
                                        numberOfLines={1}
                                        style={[s.lineName, { color: c.text }]}
                                    >
                                        {item.name} × {item.quantity}
                                    </Text>
                                    <Text
                                        style={[s.lineValue, { color: c.text }]}
                                    >
                            ...
                            <ReceiptLine
                                label="Subtotal"
                                value={money(receipt.totals.subtotal)}
                            />
                            {receipt.totals.discount > 0 && (
                                <ReceiptLine
                                    label="Discount"
                                    value={`-${money(receipt.totals.discount)}`}
                                />
                            )}
                            <ReceiptLine
                                label="Tax"
                                value={money(receipt.totals.tax)}
                            />
                            {receipt.totals.rounding !== 0 && (
                                <ReceiptLine
                                    label="Rounding"
                                    value={money(receipt.totals.rounding)}
                                />
                            )}
                            <View
                                style={[
                            ...
                            >
                                <Text style={[s.totalLabel, { color: c.text }]}>
                                    Total
                                </Text>
                                <Text
                                    style={[s.totalValue, { color: c.primary }]}
                                >
                            ...
                            {receipt.payment.changeDue ? (
                                <ReceiptLine
                                    label="Change"
                                    value={money(receipt.payment.changeDue)}
                                />
                            ) : null}
                            {receipt.payment.referenceNumber ? (
                                <ReceiptLine
                                    label="Reference"
                                    value={receipt.payment.referenceNumber}
                                />
                            ) : null}
                        </View>
                    </ScrollView>
                    <View style={s.actions}>
                        ...
                        <AppPressable
                            onPress={onClose}
                            style={[s.done, { backgroundColor: c.primary }]}
                        >
                            <X size={17} color="#FFFFFF" />
                            <Text style={[s.actionText, { color: "#FFFFFF" }]}>
                                Done
                            </Text>
                        </AppPressable>
                    </View>
