...
export function CheckoutDialog({
    ...
    useEffect(() => {
        ...
        const selected = paymentMethods.find(
            (item) => item.code === initialMethod,
        );
        setBankAccountId(
            selected?.defaultBankAccountId ??
                selected?.bankAccounts.find((item) => item.isDefault)?.id,
        );
    }, [initialMethod, paymentMethods, total, visible]);

    const selectedMethod = paymentMethods.find((item) => item.code === method);
    const selectedBankAccount = selectedMethod?.bankAccounts.find(
    ...
    const cardInvalid =
        method === "CARD" && cardLast4.length > 0 && cardLast4.length !== 4;
    const blocked = submitting || cashShort || cardInvalid;
    const paymentReference = useMemo(
        () =>
            method === "CARD"
                ? [cardLast4 ? `CARD-${cardLast4}` : "", reference.trim()]
                      .filter(Boolean)
                      .join(" · ")
                : reference.trim(),
    ...
    const confirm = () => {
        ...
        onConfirm({
            method,
            ...(method === "CASH"
                ? { tenderedAmount: tendered, changeDue }
                : {}),
        ...
        >
            ...
            >
                {!embedded && (
                    ...
                    >
                        <View>
                            ...
                            <Text
                                style={[s.subtitle, { color: c.textSecondary }]}
                            >
                                {itemCount} items to complete
                            </Text>
                        </View>
                        <AppPressable
                            accessibilityLabel={
                                embedded ? "Back to cart" : "Close checkout"
                            }
                            onPress={onClose}
                            style={[
                                s.close,
                ...
                >
                    ...
                    >
                        <Text
                            style={[s.amountLabel, { color: c.textSecondary }]}
                        >
                            Amount due
                        </Text>
                        ...
                        {paymentMethods.map(({ code, name }) => {
                            ...
                            return (
                                <AppPressable
                                    ...
                                    onPress={() => {
                                        setMethod(code);
                                        const configured = paymentMethods.find(
                                            (item) => item.code === code,
                                        );
                                        setBankAccountId(
                                            configured?.defaultBankAccountId ??
                                                configured?.bankAccounts.find(
                                                    (item) => item.isDefault,
                                                )?.id,
                                        );
                                        if (code === "CASH" && !cash)
                        ...
                        <View>
                            ...
                            >
                                ...
                                <Text
                                    style={[
                                        s.changeValue,
                                        { color: cashShort ? c.error : c.text },
                                    ]}
                                >
                                    {money(
                                        cashShort
                        ...
                        <View>
                            ...
                            {upiPaymentUri && (
                                ...
                                >
                                    ...
                                    <Text
                                        style={[s.qrTitle, { color: c.text }]}
                                    >
                                        Scan to pay {money(total)}
                                    </Text>
                                    <Text
                                        style={[
                                            s.qrHint,
