    ...
    return products.flatMap((product) => {
              ...
              ? [
                    {
                        ...
                        prescriptionRequired:
                            product.details.prescriptionRequired,
                    },
                ]
              : [];
        return source.flatMap((batch, index) => {
            ...
            const stock = Math.max(
                0,
                Math.floor(Number(batch.stock ?? product.stock) || 0),
            );
            if (!batchNumber) return [];
            return [
                {
                    id: String(
                        batch.id || `${product.id}:${batchNumber}:${index}`,
                    ),
                    ...
                    stock,
                    price:
                        batch.price == null ? undefined : Number(batch.price),
...
export function preferredProductBatch(
