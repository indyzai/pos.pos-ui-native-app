...
export type RestaurantModifier = NonNullable<
    ProductBusinessDetails["modifiers"]
>[number];
    ...
    return (details?.modifiers || []).filter((modifier) => {
        const valid =
            ...
            (modifier.price === undefined ||
                (Number.isFinite(modifier.price) && modifier.price >= 0));
...
export function toggleRestaurantModifier(
    ...
    if (selected.includes(modifier.id))
        return selected.filter((id) => id !== modifier.id);
    const remaining =
        modifier.singleSelect && modifier.group
            ? selected.filter(
                  (id) =>
                      available.find((item) => item.id === id)?.group !==
                      modifier.group,
              )
            : selected;
