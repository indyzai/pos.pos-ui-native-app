...
): BillingCalculationPolicy {
    return {
        taxCalculation:
            settings?.taxCalculation === "exclusive"
                ? "exclusive"
                : "inclusive",
...
export function calculateBillingTotals(
    ...
    const tax = money(
        items.reduce((sum, item) => {
            const line = Math.max(
                0,
                item.price * item.quantity - Math.max(0, item.discount || 0),
            );
    ...
    const total = policy.roundOffTotal
    ...
    return {
        ...
        tax,
        total,
        rounding: money(total - unroundedTotal),
    };
}
