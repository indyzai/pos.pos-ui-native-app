...
import type { ScrapPurchaseJob } from "@indyzai/feature-scrap/types";

const storageId = (scope: string, id: string) =>
    `${scope}:scrap-purchase-job:${id}`;
...
export const scrapPurchaseRepository = {
    async read(scope: string): Promise<ScrapPurchaseJob[]> {
        if (!hasNativeDatabase) {
            const rows = await readWebScopedRecords<
                ScrapPurchaseJob & { storageId: string; scope: string }
            >(webStores.scrapPurchaseJobs, scope);
            return rows.map(
                ({ storageId: _storageId, scope: _scope, ...job }) => job,
            );
        }
        initializeDatabase();
        return getDatabase()
    ...
    async replace(scope: string, jobs: ScrapPurchaseJob[]): Promise<void> {
        if (!hasNativeDatabase)
            return replaceWebScopedRecords(
