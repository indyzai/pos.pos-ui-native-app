...
export function DeviceSetupScreen({
    ...
    const submit = async () => {
        if (pin !== confirm)
            return showSnackbar("Device PIN", "PIN entries do not match.");
        setBusy(true);
        logger.info("Registering device PIN");
        try {
            await onRegister(pin);
        ...
        >
            ...
            >
                ...
                <Text style={[s.copy, { color: c.textSecondary }]}>
                    Create a 4–8 digit PIN. Future access uses Face ID,
                ...
                <TextInput
                    value={confirm}
                    onChangeText={(value) =>
                        setConfirm(value.replace(/\D/g, "").slice(0, 8))
                    }
                    keyboardType="number-pad"
                    inputMode="numeric"
                    secureTextEntry
                ...
                >
                    <Text style={s.buttonText}>
                        {busy ? "Setting up…" : "Enable device access"}
                    </Text>
                </AppPressable>
            </View>
        </SafeAreaView>
