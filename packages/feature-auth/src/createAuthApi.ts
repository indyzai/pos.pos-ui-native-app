...
export function createAuthApi(configuration: AuthApiConfiguration) {
    ...
    } = appStorageKeys[configuration.application].auth;
    ...
    function publishWebAuthorization(result: {
        type: "success" | "error";
        message?: string;
    }) {
    ...
    function openWebAuthorization(
        ...
        return new Promise((resolve, reject) => {
            ...
            const finish = (result: { type?: string; message?: string }) => {
                if (settled) return;
                settled = true;
                debugAuth("Popup authorization finished", {
                    type: result.type,
                });
    ...
    type CodeResponse = AuthResponse & {
        code?: string;
    ...
    async function fetchCurrentUser(accessToken: string): Promise<AuthUser> {
        ...
        const body = await requestJson<AuthUser & { user?: AuthUser }>(
            `${runtimeAuthApiUrl}/users/me`,
            {
                token: accessToken,
            },
        );
        ...
        if (!user.id)
            throw new Error("Your sign-in did not include a user profile.");
        return user;
    }
    ...
    async function request<T>(
        ...
        const { authApiUrl: runtimeAuthApiUrl } =
            await configuration.getRuntimeApiUrls();
        return requestJson<T>(runtimeAuthApiUrl + path, {
            method: "POST",
            body,
        });
    }
    ...
    async function getDeviceIdentifier(): Promise<string> {
        ...
        throw new Error(
            "Device authentication is available only in the installed mobile app.",
        );
    }
    ...
    async function saveSession(response: AuthResponse): Promise<void> {
        ...
        const selectedTenant =
            user.tenants?.find((tenant) => tenant.id === selectedTenantId) ??
            user.tenants?.[0];
        await Promise.all([
    ...
    async function hydrateSessionUser(): Promise<AuthUser | null> {
        ...
        const selectedTenantId = await getSessionValue(selectedTenantKey);
        const selectedTenant =
            hydratedUser.tenants?.find(
                (tenant) => tenant.id === selectedTenantId,
            ) ?? hydratedUser.tenants?.[0];
        await Promise.all([
    ...
    async function completeAuthorizationCode(
        ...
        authorizationCompletion = (async () => {
            ...
            if (!codeVerifier) {
                ...
                throw new Error(
                    "The sign-in request expired. Please start again.",
                );
            }
        ...
        try {
            ...
            await Promise.all([
                ...
                deleteSessionValue(oauthVerifierKey),
            ]);
    ...
    const authApi = {
        ...
        async getApplicationRedirectUrl(appName: string): Promise<string> {
            ...
            const configured =
                redirects?.[environment] ?? redirects?.prod ?? redirects?.dev;
            const url =
                typeof configured === "string" ? configured : configured?.url;
            if (!url)
                throw new Error(
                    `Redirect URL is not configured for ${appName}.`,
                );
        ...
        async getDeviceRegistrationDetails(): Promise<DeviceRegistrationDetails> {
            ...
            return {
                ...
                applicationId:
                    ...
                    (Platform.OS === "web" ? authAppId : "Unavailable"),
                applicationVersion:
        ...
        async selectTenant(tenantId: string): Promise<AuthTenant> {
            ...
            if (!tenant)
                throw new Error(
                    "That business is no longer available for this account.",
                );
        ...
        async registerDevice(pin: string): Promise<void> {
            if (Platform.OS === "web")
                throw new Error("Device authentication is disabled on web.");
            if (!/^\d{4,8}$/.test(pin))
                throw new Error("Choose a 4 to 8 digit device PIN.");
            const accessToken = await getSessionValue(accessTokenKey);
            if (!accessToken)
                throw new Error("Sign in before registering this device.");
            ...
            try {
                ...
                await Promise.all([
                    ...
                    SecureStore.deleteItemAsync(deviceTokenKey).catch(
                        () => undefined,
                    ),
                ]);
                throw error;
        ...
        async login({ email, password }: LoginCredentials) {
            ...
            if (!result.code)
                throw new Error(
                    "The authentication service did not return a login code.",
                );
        ...
        async authorize(provider: "google" | "microsoft") {
            debugAuth("Starting social authorization", {
                provider,
                platform: Platform.OS,
            });
            const popup =
                Platform.OS === "web"
                    ? window.open(
                          "about:blank",
                          `${authAppId}-authentication`,
                          "popup=yes,width=500,height=650",
                      )
                    : null;
            ...
            const discovery = {
                authorizationEndpoint: `${runtimeAuthApiUrl}/auth/${provider}`,
            };
            ...
            if (!request.codeVerifier)
                throw new Error(
                    "Sign-in could not create a secure verification code.",
                );
            await Promise.all([
            ...
            if (Platform.OS === "web") {
                debugAuth("Launching prepared authorization request", {
                    provider,
                });
                return openWebAuthorization(
                    await request.makeAuthUrlAsync(discovery),
                    popup!,
                );
            }
            ...
            if (result.type !== "success")
                throw new Error(
                    `${provider === "google" ? "Google" : "Microsoft"} sign-in did not complete.`,
                );
        ...
        async checkHealth(
            ...
            try {
                const { authApiUrl: runtimeAuthApiUrl } =
                    await configuration.getRuntimeApiUrls();
                const healthUrl = runtimeAuthApiUrl.replace(
                    /\/api\/v1\/?$/,
                    "/api/health",
                );
                let timer: any = null;
