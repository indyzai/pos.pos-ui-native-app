...
async function readRecords<T>(
    ...
    if (!hasNativeDatabase) {
        ...
        >(storeName, scope);
        return rows.map(
            ({ storageId: _id, scope: _scope, ...record }) => record as T,
        );
    }
    initializeDatabase();
    return getDatabase()
...
async function replaceRecords<T extends { id: string }>(
    ...
    if (!hasNativeDatabase)
        return replaceWebScopedRecords(storeName, scope, records);
    initializeDatabase();
...
export const ordersRepository = {
    readOrders: (scope: string) =>
        readRecords<SalesOrder>(scope, orders, webStores.orders),
    replaceOrders: (scope: string, records: SalesOrder[]) =>
        replaceRecords(scope, records, orders, webStores.orders),
    readRefunds: (scope: string) =>
        readRecords<RefundRecord>(scope, refunds, webStores.refunds),
