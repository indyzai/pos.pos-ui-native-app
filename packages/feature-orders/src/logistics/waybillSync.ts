    ...
    while (jobs.length) {
        ...
        try {
            const result = await request<{
                createLogisticsRecord: { id: string };
            }>(createWaybillMutation, {
                input: job.payload,
            });
            if (!result.createLogisticsRecord?.id)
                throw new Error("Server did not acknowledge the waybill.");
        } catch (error) {
            job.status = "FAILED";
            job.error =
                error instanceof Error
                    ? error.message
                    : "Unable to sync waybill.";
            await persist();
            throw error;
        }
        jobs.shift();
