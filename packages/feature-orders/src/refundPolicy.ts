...
export function buildPendingRefund(
    ...
    if (
        ...
        throw new Error(
            "A product can only appear once in a refund selection.",
        );
    }
        ...
        return [
            {
                ...
                refundAmount:
                    Math.round(
                        (sold.lineTotal - sold.discountAmount) * ratio * 100,
                    ) / 100,
    ...
    const subtotal =
        Math.round(
            items.reduce((sum, item) => sum + item.refundAmount, 0) * 100,
        ) / 100;
    const tax =
        Math.round(items.reduce((sum, item) => sum + item.taxAmount, 0) * 100) /
        100;
    return {
        id: offlineId,
        offlineId,
