...
import type { CounterPrinter } from "@indyzai/feature-printers/types";
...
export async function readPrinters(scope: string): Promise<CounterPrinter[]> {
    if (!hasNativeDatabase) {
        const rows = await readWebScopedRecords<
            CounterPrinter & { storageId: string; scope: string }
        >(webStores.printers, scope);
        return rows.map(
            ({ storageId: _storageId, scope: _scope, ...printer }) => printer,
        );
    }
    initializeDatabase();
    return getDatabase()
...
export async function replacePrinters(
    scope: string,
    records: CounterPrinter[],
): Promise<void> {
    if (!hasNativeDatabase)
        return replaceWebScopedRecords(webStores.printers, scope, records);
    initializeDatabase();
