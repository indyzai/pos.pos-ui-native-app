...
import { printJobs } from "@indyzai/pos-database/schema";
import {
    readWebPrintJobs,
    writeWebPrintJob,
} from "@indyzai/pos-database/web-client";
import type { LocalPrintJob } from "@indyzai/feature-printers/types";
...
export async function listPendingPrintJobs(
    ...
    if (!hasNativeDatabase) {
        const rows = await readWebPrintJobs<
            LocalPrintJob & { scope: string; storageId: string }
        >(scope);
        return rows
    ...
    return getDatabase()
        ...
        .from(printJobs)
        .where(
            and(eq(printJobs.scope, scope), ne(printJobs.status, "COMPLETED")),
        )
        .all()
        .flatMap((row) => {
            try {
...
export async function savePrintJob(
    scope: string,
    job: LocalPrintJob,
): Promise<void> {
