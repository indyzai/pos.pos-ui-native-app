...
type CatalogRow = {
    ...
    category?: {
        name?: string | null;
        type?: Product["categoryType"] | null;
    } | null;
    tax?: { percentage?: number | null; rate?: number | null } | null;
};

export type ProductRequest = <T>(
    query: string,
...
function toProduct(row: CatalogRow): Product {
    const details = (row.details || undefined) as
        ProductBusinessDetails | undefined;
    return {
        id: String(row.id),
        name: row.name,
...
export async function fetchCatalog(
    ...
    for (let skip = 0; ; skip += 100) {
        const data = await request<{ products: CatalogRow[] }>(
            catalogQuery(categoryType),
            { skip, take: 100 },
        );
        rows.push(...data.products);
        if (data.products.length < 100)
            return rows
                .filter((row) => row.category?.type === categoryType)
                .map(toProduct);
    }
}
