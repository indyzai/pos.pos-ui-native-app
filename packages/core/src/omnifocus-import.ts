...
const buildWarnings = (counters: OmniFocusWarningCounters): string[] => {
    ...
    appendWarning(
        ...
        '1 nested OmniFocus task was flattened because OpenPOS cannot preserve its hierarchy directly.',
        '{count} nested OmniFocus tasks were flattened because OpenPOS cannot preserve their hierarchy directly.'
    );
    appendWarning(
        warnings,
...
const parseJsonImport = (
    ...
    rawProjects.forEach((project) => {
        ...
        const repeatNote = rootTask?.repetition
            ? `OmniFocus project repeat rule: ${readStringValue((rootTask.repetition as Record<string, unknown>)?.ruleString)
    ...
    rawTasks.forEach((task) => {
        ...
        const repeatNote = task.repetition
            ? `OmniFocus project repeat rule: ${readStringValue((task.repetition as Record<string, unknown>)?.ruleString)
