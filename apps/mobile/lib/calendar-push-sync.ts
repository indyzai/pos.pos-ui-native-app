...
const CALENDAR_PUSH_ENABLED_KEY = 'openpos:calendar-push-sync:enabled';
const CALENDAR_ID_KEY = 'openpos:calendar-push-sync:calendar-id';
const CALENDAR_TARGET_ID_KEY = 'openpos:calendar-push-sync:target-calendar-id';
const CALENDAR_COLOR_KEY = 'openpos:calendar-push-sync:color';
const PLATFORM = Platform.OS;
const MANAGED_CALENDAR_TITLE = 'OpenPOS';
const MANAGED_CALENDAR_NAME = 'openpos';
...
function isStoredOpenPOSManagedCalendar(calendar: Calendar.Calendar, storedCalendarId: string | null): boolean {
    return Boolean(storedCalendarId && calendar.id === storedCalendarId);
}

function isAppCreatedOpenPOSCalendar(calendar: Calendar.Calendar): boolean {
    const title = getCalendarDisplayName(calendar).trim().toLowerCase();
    const name = typeof calendar.name === 'string' ? calendar.name.trim().toLowerCase() : '';
    return title === MANAGED_CALENDAR_TITLE.toLowerCase() && name === MANAGED_CALENDAR_NAME;
}
...
export const getCalendarPushTargetCalendars = async (): Promise<CalendarPushTargetCalendar[]> => {
    try {
        ...
        return calendars
            ...
            .map((calendar) => {
                const isOpenPOSDedicated = isOpenPOSNamedCalendar(calendar);
                return {
                    id: calendar.id,
                    ...
                    isOpenPOSDedicated,
                    isOpenPOSManaged: isStoredOpenPOSManagedCalendar(calendar, storedCalendarId),
                    isLocalOnly: isLocalOnlyCalendar(calendar),
                };
            })
            .sort((a, b) => {
                if (a.isOpenPOSManaged !== b.isOpenPOSManaged) return a.isOpenPOSManaged ? -1 : 1;
                if (a.isOpenPOSDedicated !== b.isOpenPOSDedicated) return a.isOpenPOSDedicated ? -1 : 1;
...
/**
 * Returns the ID of the managed "OpenPOS" calendar, creating it if needed.
 * Returns null if the calendar cannot be created (e.g. no permission, no source).
 */
export const ensureOpenPOSCalendar = async (): Promise<string | null> => {
    try {
        ...
        if (Platform.OS === 'android') {
            ...
            if (!androidSeed) {
                void logWarn('No owned Android calendar source available; cannot create OpenPOS calendar', {
                    scope: 'calendar-push',
            ...
            if (!source) {
                void logWarn('No calendar source available; cannot create OpenPOS calendar', {
                    scope: 'calendar-push',
                });
                return null;
            }
        ...
        void logInfo('Created OpenPOS calendar', {
            scope: 'calendar-push',
            extra: { calendarId: newId },
        });
        return newId;
    } catch (error) {
        void logError(error, { scope: 'calendar-push', extra: { operation: 'ensureOpenPOSCalendar' } });
        return null;
    }
};

export const updateOpenPOSCalendarColor = async (color: string): Promise<boolean> => {
    ...
    try {
        ...
        const target = calendars.find((calendar) => storedCalendarId && calendar.id === storedCalendarId)
            ?? calendars.find(isAppCreatedOpenPOSCalendar);
        ...
        if (Platform.OS === 'android') {
            return await recreateManagedOpenPOSCalendar();
        }

        await Calendar.updateCalendarAsync(target.id, { color: normalized });
        return true;
    } catch (error) {
        void logWarn('Failed to update OpenPOS calendar color', {
            scope: 'calendar-push',
...
/**
 * Deletes and recreates the managed "OpenPOS" calendar so a color change takes
 ...
 */
async function recreateManagedOpenPOSCalendar(): Promise<boolean> {
    let recreatedId: string | null = null;
    await enqueueCalendarSync(async () => {
        await deleteOpenPOSCalendar();
        recreatedId = await ensureOpenPOSCalendar();
...
async function resolveCalendarPushTarget(): Promise<CalendarPushTarget | null> {
    ...
    if (selectedId) {
        try {
            ...
            void logWarn('Selected calendar push target is unavailable; falling back to OpenPOS calendar', {
                scope: 'calendar-push',
    ...
    const managedId = await ensureOpenPOSCalendar();
    return managedId ? { id: managedId } : null;
}

/**
 * Deletes the managed OpenPOS calendar and removes the stored ID.
 * Called when the user disables calendar push sync and chooses to clean up.
 */
export const deleteOpenPOSCalendar = async (): Promise<void> => {
    const storedId = await getStoredCalendarId();
    ...
    try {
        ...
        calendars.forEach((calendar) => {
            if (isAppCreatedOpenPOSCalendar(calendar)) {
                calendarIdsToDelete.add(calendar.id);
            }
        });
    } catch (error) {
        void logWarn('Failed to inspect calendars before deleting OpenPOS calendar', {
            scope: 'calendar-push',
    ...
    if (calendarIdsToDelete.size === 0) {
        ...
        void logInfo('Deleted OpenPOS calendar', {
            scope: 'calendar-push',
    ...
    try {
        ...
        void logWarn('Failed to clear deleted OpenPOS calendar sync entries', {
            scope: 'calendar-push',
            extra: { error: String(error) },
        });
    }
        ...
        nameNotifyListener('calendar-push', (currentTasks: Task[]) => {
            ...
            for (const task of currentTasks) {
                ...
                if (
                    ...
                    prev.suppressOpenPOSReminders !== task.suppressOpenPOSReminders ||
