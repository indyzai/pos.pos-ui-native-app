...
export const ANDROID_CONTEXT_ACTIVATE_ACTION = 'com.indyzai.pos.openpos.action.ACTIVATE_CONTEXT';
export const ANDROID_CONTEXT_DEACTIVATE_ACTION = 'com.indyzai.pos.openpos.action.DEACTIVATE_CONTEXT';
...
export function parseContextAutomationUrl(rawUrl: string): ContextAutomationPayload | null {
  ...
  if ((parsed.protocol || '').toLowerCase() !== 'openpos:') return null;
...
export function buildContextAutomationNotificationCopy(
  ...
  if (count === 0) {
    return {
      ...
      message: interpolate(templates.noTasksMessage ?? 'OpenPOS did not find any /next tasks for {{context}}.'),
    };
  }
