...
import type { MergeStats } from '@openpos/core';
...
describe('mobile sync-service test utils', () => {
  ...
  it('formats iOS temporary inbox file sync errors with provider guidance', () => {
    const error = new Error("Calling the 'writeAsStringAsync' function has failed -> File '/private/var/mobile/.../tmp/com.indyzai.pos.openpos-Inbox/data.json.tmp' is not writable");
  ...
  it('formats writable file sync errors with actionable text', () => {
    const error = new Error("File '/var/mobile/Containers/Data/Application/abc/Documents/OpenPOS/data.json.tmp' is not writable");
    const message = formatSyncErrorMessage(error, 'file');
  ...
  it('detects sync file paths and resolves base directory', () => {
    ...
    expect(getFileSyncBaseDir('file:///var/mobile/Containers/Shared/AppGroup/openpos-backup-2026-02-25.json')).toBe('file:///var/mobile/Containers/Shared/AppGroup');
  ...
  it('detects likely file paths for custom sync filenames', () => {
    ...
    expect(isLikelyFilePath('file:///var/mobile/Containers/Shared/AppGroup/openpos-backup-2026-02-25.json')).toBe(true);
    expect(isLikelyFilePath('/storage/folder')).toBe(false);
    expect(isLikelyFilePath('/storage/folder/')).toBe(false);
  });

  it('normalizes legacy iOS absolute sync paths to file uri', () => {
    expect(normalizeFileSyncPath('/var/mobile/Containers/Data/Application/abc/Documents/OpenPOS/data.json', 'ios'))
      .toBe('file:///var/mobile/Containers/Data/Application/abc/Documents/OpenPOS/data.json');
    expect(normalizeFileSyncPath('file:///var/mobile/Containers/Data/Application/abc/Documents/OpenPOS/data.json', 'ios'))
      .toBe('file:///var/mobile/Containers/Data/Application/abc/Documents/OpenPOS/data.json');
  ...
  it('classifies unavailable File Sync locking separately for localized recovery guidance', () => {
    expect(classifySyncFailure(
      'Safe File Sync locking is unavailable for this location. Re-select the sync folder, restart OpenPOS, or use WebDAV.',
