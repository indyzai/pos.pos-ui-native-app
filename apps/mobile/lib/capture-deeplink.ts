...
const normalizeRouteFromUrl = (url: URL): string => {
    // openpos://capture -> hostname "capture"
    // openpos:///capture -> pathname "/capture"
...
export function isShortcutCaptureUrl(rawUrl: string): boolean {
    ...
    try {
        ...
        return (parsed.protocol || '').toLowerCase() === 'openpos:' && normalizeRouteFromUrl(parsed) === 'capture';
    } catch {
        return false;
    }
}
...
export function isOpenFeatureUrl(rawUrl: string): boolean {
    ...
    try {
        const parsed = new URL(rawUrl);
        return (parsed.protocol || '').toLowerCase() === 'openpos:' && normalizeRouteFromUrl(parsed) === 'open-feature';
    } catch {
        return false;
    }
}
...
// openpos://open?task=<id> | ?project=<id> | ?area=<id> — the deep link a
...
export function isEntityOpenUrl(rawUrl: string): boolean {
    ...
    try {
        const parsed = new URL(rawUrl);
        return (parsed.protocol || '').toLowerCase() === 'openpos:' && normalizeRouteFromUrl(parsed) === 'open';
    } catch {
        return false;
    }
}
