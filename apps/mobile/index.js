...
const installKeepAwakeActivationGuard = () => {
  try {
    ...
    if (keepAwakeModule.__openposActivateWrapped) {
      return;
    }
    ...
    keepAwakeModule.activate = async (...args) => {
      try {
        ...
        if (details.includes('Unable to activate keep awake')) {
          ...
          console.warn('[OpenPOSStartup] keep-awake activation skipped until activity is ready');
          return;
        }
        throw error;
      }
    };
    keepAwakeModule.__openposActivateWrapped = true;
    ...
    console.warn(`[OpenPOSStartup] keep-awake guard install failed: ${details}`);
  }
};
...
const loadWidgetHandler = () => {
  ...
  try {
    ...
    console.error(`[OpenPOSStartup] phase=js.index.widget_handler_failed_error details=${details}`);
  }
};
...
try {
  ...
  console.error(`[OpenPOSStartup] phase=js.index.expo_router_entry_failed_error details=${details}`);
  throw error;
}
