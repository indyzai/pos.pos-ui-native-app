...
private let openposFocusLockWidgetKind = "OpenPOSFocusLockWidget"

// Lock screen (accessory family) widget showing the current focused task (#821).
// Reuses OpenPOSTasksWidgetProvider: accessory families fall through to the
...
struct OpenPOSFocusLockWidget: Widget {
    let kind: String = openposFocusLockWidgetKind
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: OpenPOSTasksWidgetProvider()) { entry in
            OpenPOSFocusLockWidgetEntryView(entry: entry)
        }
...
private struct OpenPOSFocusLockWidgetEntryView: View {
    let entry: OpenPOSTasksWidgetEntry

    var body: some View {
        if #available(iOSApplicationExtension 16.0, *) {
            OpenPOSFocusLockView(entry: entry)
        } else {
            EmptyView()
        }
    }
...
private struct OpenPOSFocusLockView: View {
    let entry: OpenPOSTasksWidgetEntry
...
private extension View {
    @ViewBuilder
    func openposLockWidgetBackground() -> some View {
