...
public:
  ScopedUtfChars(JNIEnv *env, jstring value)
      : env_(env), value_(value),
        chars_(env->GetStringUTFChars(value, nullptr)) {}
  ~ScopedUtfChars() {
    if (chars_ != nullptr)
...
bool parse_identity(
  ...
  errno = 0;
  const unsigned long long device =
      strtoull(device_text.c_str(), &device_end, 10);
  if (errno != 0 || device_end == nullptr || *device_end != '\0')
    return false;
  errno = 0;
  const unsigned long long inode = strtoull(inode_text.c_str(), &inode_end, 10);
  if (errno != 0 || inode_end == nullptr || *inode_end != '\0')
    return false;
  *output = {static_cast<uint64_t>(device), static_cast<uint64_t>(inode)};
  return true;
}
...
Java_tech_indyzai_openpos_attachmentfileinstaller_ExactAttachmentPublisherNative_publishRelativeNoReplace(
    JNIEnv *env, jobject, jint source_fd, jstring private_directory_path,
    jstring target_directory_path, jstring target_name,
  ...
  if (private_path.get() == nullptr || target_path.get() == nullptr ||
      target_leaf.get() == nullptr || expected_source.get() == nullptr ||
      expected_private.get() == nullptr || expected_target.get() == nullptr) {
    return JNI_FALSE;
  }
  const std::string target_leaf_value(target_leaf.get());
  if (target_leaf_value.empty() || target_leaf_value == "." ||
      target_leaf_value == ".." ||
  ...
  const int private_fd =
      open(private_path.get(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
  if (private_fd < 0) {
    throw_io_exception(
        env, std::string("Could not retain private publication directory: ") +
                 strerror(errno));
    return JNI_FALSE;
  }
  const int target_fd =
      open(target_path.get(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
  if (target_fd < 0) {
    const int saved = errno;
    close(private_fd);
    throw_io_exception(
        env, std::string("Could not retain attachment target directory: ") +
                 strerror(saved));
    return JNI_FALSE;
  }
  ...
  const bool exact_named_stage =
      fstat(source_fd, &opened) == 0 && S_ISREG(opened.st_mode) &&
  ...
  if (!exact_named_stage) {
    ...
    close(private_fd);
    throw_io_exception(
        env, "Verified attachment stage name changed before publication");
    return JNI_FALSE;
  }

  long result = syscall(SYS_renameat2, private_fd, "stage", target_fd,
                        target_leaf.get(), RENAME_NOREPLACE);
  if (result != 0 && rename_noreplace_needs_exact_handle_fallback(errno)) {
    const std::string exact_source =
        "/proc/self/fd/" + std::to_string(source_fd);
    result = linkat(AT_FDCWD, exact_source.c_str(), target_fd,
                    target_leaf.get(), AT_SYMLINK_FOLLOW);
    if (result == 0 && unlinkat(private_fd, "stage", 0) != 0) {
      const int saved = errno;
      close(target_fd);
      close(private_fd);
      throw_io_exception(env, std::string("Published attachment stage could "
                                          "not release its private name: ") +
                                  strerror(saved));
      return JNI_FALSE;
    }
  }
  if (result != 0) {
    ...
    if (saved == EEXIST)
      return JNI_FALSE;
    throw_io_exception(
        env, std::string("Could not publish exact attachment stage: ") +
                 strerror(saved));
    return JNI_FALSE;
  }
  ...
  if (!durable) {
    throw_io_exception(
        env,
        std::string("Could not flush attachment publication directories: ") +
            strerror(saved));
    return JNI_FALSE;
  }
  return JNI_TRUE;
...
Java_tech_indyzai_openpos_attachmentfileinstaller_ExactAttachmentPublisherNative_retireEmptyDirectoryIfIdentity(
    JNIEnv *env, jobject, jstring parent_directory_path, jstring directory_name,
    jstring expected_directory_identity, jstring expected_parent_identity) {
  ...
  if (parent_path.get() == nullptr || directory_leaf.get() == nullptr ||
    ...
    return static_cast<jint>(openpos::attachment_file_installer::
                                 DirectoryRetirementResult::kIoError);
  }
  const std::string directory_leaf_value(directory_leaf.get());
  if (directory_leaf_value.empty() || directory_leaf_value == "." ||
      directory_leaf_value == ".." ||
  ...
  if (!parse_identity(expected_directory.get(), &directory_identity) ||
      !parse_identity(expected_parent.get(), &parent_identity)) {
    throw_io_exception(env,
                       "Attachment publication directory identity is invalid");
    return static_cast<jint>(openpos::attachment_file_installer::
                                 DirectoryRetirementResult::kIoError);
  }

  const int parent_fd =
      open(parent_path.get(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
  if (parent_fd < 0) {
    throw_io_exception(
        env, std::string("Could not retain attachment publication parent: ") +
                 strerror(errno));
    return static_cast<jint>(openpos::attachment_file_installer::
                                 DirectoryRetirementResult::kIoError);
  }
  int error_number = 0;
  const auto result =
      openpos::attachment_file_installer::retire_empty_directory_if_identity(
          parent_fd, directory_leaf.get(), directory_identity, parent_identity,
  ...
  if (result ==
    ...
    throw_io_exception(
        env,
        std::string(
            "Could not retire private attachment publication directory: ") +
...
Java_tech_indyzai_openpos_attachmentfileinstaller_ExactAttachmentPublisherNative_retireReservedPrivateStage(
    JNIEnv *env, jobject, jstring parent_directory_path,
  ...
  if (parent_fd < 0) {
    throw_io_exception(
        env, std::string(
                 "Could not retain reserved attachment publication parent: ") +
                 strerror(errno));
    return static_cast<jint>(openpos::attachment_file_installer::
                                 DirectoryRetirementResult::kIoError);
  }
  int error_number = 0;
  const auto result =
      openpos::attachment_file_installer::retire_reserved_private_stage(
          parent_fd, directory_leaf.get(), &error_number);
  close(parent_fd);
  if (result ==
      openpos::attachment_file_installer::DirectoryRetirementResult::kIoError) {
    throw_io_exception(
