...
const applyPackageManifestChanges = (pkg, enableExpoBuildFromSource) => {
  ...
  if (pkg.dependencies && pkg.dependencies['@openpos/core'] === 'workspace:*') {
    pkg.dependencies['@openpos/core'] = 'file:../../packages/core';
    changes.push('rewrote @openpos/core to file:../../packages/core for npm compatibility');
  }
...
const run = () => {
  ...
  const coreDep = pkg.dependencies?.['@openpos/core'];
  if (typeof coreDep === 'string' && coreDep.startsWith('workspace:')) {
    throw new Error('[fdroid] @openpos/core still uses workspace:*; npm install will fail in non-workspace environments');
  }
};
