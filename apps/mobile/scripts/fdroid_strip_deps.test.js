...
describe('fdroid_strip_deps', () => {
  it('strips non-FOSS package dependencies and keeps npm-compatible workspace deps', () => {
    ...
    expect(pkg.dependencies['@openpos/core']).toBe('file:../../packages/core');
    ...
    expect(changes).toContain('rewrote @openpos/core to file:../../packages/core for npm compatibility');
  });
  ...
  it('removes Google-backed native module sources from the F-Droid prep tree', () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'openpos-fdroid-strip-'));
