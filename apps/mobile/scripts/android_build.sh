...
if [[ "${FOSS_BUILD}" == "1" ]]; then
  ...
  export FEEDBACK_ENDPOINT_URL="${FEEDBACK_ENDPOINT_URL:-https://feedback.openpos.app}"
...
for arch in "${ARCH_LIST[@]}"; do
  ...
  if [[ -n "$apk_path" ]]; then
    out_name="openpos-${VERSION}-${arch_trimmed}${SUFFIX}.apk"
...
if [[ "$found" -eq 0 ]]; then
  ...
  out_name="openpos-${VERSION}-universal${SUFFIX}.apk"
  cp "$apk_path" "${OUTPUT_DIR}/${out_name}"
  echo "APK: ${OUTPUT_DIR}/${out_name}"
fi
