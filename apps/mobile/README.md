...
React Native mobile app for the OpenPOS productivity system.
...
- TypeScript
- Zustand (shared with desktop via @openpos/core)
...
OpenPOS builds **split APKs per ABI** (arm64-v8a, armeabi-v7a, x86, x86_64) so the arm64 file stays under store size limits like F-Droid/Izzy.
...
```
openpos-<version>-arm64-v8a.apk
```

The build script may also write the raw Gradle output (e.g. `app-arm64-v8a-release.apk`), but you only need the `openpos-<version>-arm64-v8a.apk` file for releases.
...
gh release upload vX.Y.Z build/openpos-<version>-arm64-v8a.apk --clobber

# Or create new release with APK
gh release create vX.Y.Z build/openpos-<version>-arm64-v8a.apk --title "vX.Y.Z" --notes "Release notes here"
...
This enables JS startup markers (`[OpenPOSStartup] ...`) while keeping normal builds quiet.
...
PACKAGE=com.indyzai.pos.openpos ACTIVITY=.MainActivity bash apps/mobile/scripts/android_startup_benchmark.sh
```
...
```bash
adb shell perfetto -o /data/misc/perfetto-traces/openpos-startup.pftrace -t 12s \
  sched freq idle am wm gfx view binder_driver hal dalvik input res memory
adb pull /data/misc/perfetto-traces/openpos-startup.pftrace
```

Then open https://ui.perfetto.dev and correlate `OpenPOSStartup` log phases with main-thread blocking, I/O, and GC sections.

## Data Storage

Tasks are stored in AsyncStorage and synced via the shared @openpos/core package.
...
### WebDAV / Cloud
OpenPOS also supports WebDAV and Cloud sync backends in **Settings → Sync**:
...
3. Add redirect URI: `openpos://redirect`.
...
- `/Apps/OpenPOS/data.json`
- `/Apps/OpenPOS/attachments/*` (file attachments)
