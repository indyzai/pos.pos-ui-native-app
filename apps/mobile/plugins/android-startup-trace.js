...
private const val STARTUP_TAG = "OpenPOSStartup"
...
inline fun <T> startupSection(phase: String, block: () -> T): T {
  ...
  Trace.beginSection("OpenPOS.$phase")
  try {
    return block()
  } finally {
    Trace.endSection()
...
const contextAutomationIntentFunction = `  private fun normalizeContextAutomationIntent(intent: Intent?) {
    val contextAction = when (intent?.action) {
      "com.indyzai.pos.openpos.action.ACTIVATE_CONTEXT" -> "activate"
      "com.indyzai.pos.openpos.action.DEACTIVATE_CONTEXT" -> "deactivate"
      else -> return
    }
