...
function setupEnabled(calendarId = 'cal-1', targetCalendarId: string | null = null) {
    ...
    if (!targetCalendarId) {
        mockGetItem.mockResolvedValueOnce(calendarId); // ensureOpenPOSCalendar → stored ID
    }
}
...
function setupStatefulAndroidCalendar() {
    const storage = new Map<string, string>([
        ['openpos:calendar-push-sync:enabled', '1'],
        ['openpos:calendar-push-sync:calendar-id', 'cal-old'],
        ['openpos:calendar-push-sync:color', '#3B82F6'],
    ]);
    ...
    let calendars: MockAndroidCalendar[] = [
        { id: 'cal-old', title: 'OpenPOS', name: 'openpos', color: '#3B82F6', ...ownedAccount },
    ...
    mockCreateCalendarAsync.mockImplementation(async (details?: { color?: string }) => {
        calendars = [...calendars, { id: 'cal-new', title: 'OpenPOS', name: 'openpos', color: details?.color, ...ownedAccount }];
        return 'cal-new';
    });
...
beforeEach(() => {
    ...
    mockGetCalendarsAsync.mockResolvedValue([{ id: 'cal-1', title: 'OpenPOS' }]);
...
describe('ensureOpenPOSCalendar', () => {
    it('returns the stored calendar ID when the calendar still exists', async () => {
        ...
        const id = await ensureOpenPOSCalendar();
    ...
    it('recreates the calendar when the stored one has been deleted', async () => {
        ...
        expect(mockSetItem).toHaveBeenCalledWith('openpos:calendar-push-sync:calendar-id', 'cal-2');
    });

    it('uses the saved OpenPOS calendar color when creating the calendar', async () => {
        mockGetItem.mockImplementation(async (key: string) => {
            if (key === 'openpos:calendar-push-sync:calendar-id') return null;
            if (key === 'openpos:calendar-push-sync:color') return '#DB2777';
            return null;
        });
...
describe('calendar push color', () => {
    it('normalizes saved colors and updates the managed OpenPOS calendar when supported', async () => {
        mockGetItem.mockImplementation(async (key: string) => {
            if (key === 'openpos:calendar-push-sync:calendar-id') return 'cal-1';
            if (key === 'openpos:calendar-push-sync:color') return '#db2777';
            return null;
        });
        mockGetCalendarsAsync.mockResolvedValue([{ id: 'cal-1', title: 'OpenPOS', allowsModifications: true }]);

        await expect(getCalendarPushColor()).resolves.toBe('#DB2777');
        const updated = await updateOpenPOSCalendarColor('#059669');

        expect(updated).toBe(true);
        expect(mockSetItem).toHaveBeenCalledWith('openpos:calendar-push-sync:color', '#059669');
    ...
    it('recreates the managed calendar with the new color on Android so external apps update (#726)', async () => {
        ...
        expect(mockSetItem).toHaveBeenCalledWith('openpos:calendar-push-sync:calendar-id', 'cal-new');
    });
    ...
    it('re-pushes events to the recreated Android calendar so they inherit the new color (#726)', async () => {
        ...
        await updateOpenPOSCalendarColor('#059669');
    ...
    it('stores the color but does not recreate when no managed Android calendar exists yet (#726)', async () => {
        mockPlatform.OS = 'android';
        mockGetItem.mockImplementation(async (key: string) => (
            key === 'openpos:calendar-push-sync:color' ? '#3B82F6' : null
        ));
        mockGetCalendarsAsync.mockResolvedValue([
    ...
    it('lists writable device calendars and filters read-only calendars', async () => {
        ...
        expect(targets).toEqual(expect.arrayContaining([
            expect.objectContaining({
                id: 'managed-local',
                name: 'OpenPOS',
                isOpenPOSDedicated: true,
                isOpenPOSManaged: true,
                isLocalOnly: true,
            }),
            expect.objectContaining({
                id: 'google-openpos',
...
describe('deleteOpenPOSCalendar', () => {
    it('removes app-created OpenPOS calendars even when the stored calendar id was lost', async () => {
        mockGetItem
        ...
        expect(mockRemoveItem).toHaveBeenCalledWith('openpos:calendar-push-sync:calendar-id');
    });

    it('clears the selected target and sync rows for deleted OpenPOS calendars', async () => {
        mockGetItem
        ...
        expect(mockRemoveItem).toHaveBeenCalledWith('openpos:calendar-push-sync:target-calendar-id');
    ...
    it('pushes project, section, status, effort, and links into the event notes', async () => {
        ...
        const task = makeTask({
            ...
            attachments: [
                ...
                { id: 'att-2', kind: 'link', uri: 'openpos://task/other' },
            ],
        });
...
describe('runFullCalendarSync — selected target calendar', () => {
    ...
    it('keeps titles unprefixed when the selected target is the managed OpenPOS calendar', async () => {
    ...
    it('preserves task titles that already start with the OpenPOS prefix', async () => {
        setupEnabled('cal-managed', 'google-primary');
        mockGetCalendarsAsync.mockResolvedValue([
            {
                id: 'google-primary',
        ...
        const task = makeTask({ title: 'OpenPOS: Existing prefix' });
        setStoreTasks([task]);
    ...
    it('keeps titles unprefixed when the selected target is a user-created Google OpenPOS calendar', async () => {
        setupEnabled('cal-managed', 'google-openpos');
        mockGetCalendarsAsync.mockResolvedValue([
            {
                id: 'google-openpos',
                title: 'OpenPOS',
        ...
        expect(mockCreateEventAsync).toHaveBeenCalledWith('google-openpos', expect.objectContaining({
            calendarId: 'google-openpos',
            title: task.title,
        }));
    });
