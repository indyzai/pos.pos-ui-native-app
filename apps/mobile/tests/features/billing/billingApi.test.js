  ...
  globalThis.fetch = async (_url, options) => {
            ...
            products: [
              {
                ...
                category: { name: 'Beverages', type: 'SERVICE' },
                tax: { percentage: 5 },
              },
            ],
          },
    ...
    if (query.includes('BillingTechnicians')) {
      return {
        ...
        json: async () => ({
          data: {
            parties: [{ id: 8, name: 'Technician A', phone: '123', details: { specialization: 'Repair' } }],
          },
        }),
      };
    }
    if (query.includes('BillingTaxRates')) {
      return {
        ok: true,
        json: async () => ({ data: { taxes: [{ id: 5, name: 'GST 5%', percentage: 5, isActive: true }] } }),
      };
    }
    return {
      ok: true,
