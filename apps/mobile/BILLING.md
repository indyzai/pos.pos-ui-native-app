...
Billing sends the signed-in access token and selected business in `x-tenant-id`. Products (including category/tax) are paginated from GraphQL. The active counter session comes from `organization.activeSession`. Open a counter using POS web before first use; opening/closing counters is not included in this mobile flow.
...
Card/UPI selection records a payment already collected externally; it is not a payment gateway. A cached session can be closed remotely while offline; rejected bills remain queued and require operator reconciliation. Do not clear app data or uninstall while bills are pending. Expo SQLite data is durable local storage, not encrypted credential storage. Cart drafts are not persisted.
