...
class SqliteCollection<T extends LocalRecord> implements CollectionRepository<T> {
  ...
  private getColumns() {
    return (this.columns ??= this.sqlite.getAllSync<Column>(`PRAGMA table_info(${quote(this.name)})`));
  }
  ...
  async list(query: LocalQuery = {}) {
    ...
    if (!query.includeDeleted && columns.has('deleted_at')) where.push('deleted_at IS NULL');
  ...
  async putMany(records: readonly T[]) {
    this.sqlite.withTransactionSync(() => records.forEach((record) => void this.put(record)));
  }

  async remove(id: string) {
    this.sqlite.runSync(`DELETE FROM ${quote(this.name)} WHERE id = ? AND scope = ?`, id, this.scopeKey);
  }

  async replace(records: readonly T[]) {
    records.forEach((record) => this.assertScope(record));
    this.sqlite.withTransactionSync(() => {
      this.sqlite.runSync(`DELETE FROM ${quote(this.name)} WHERE scope = ?`, this.scopeKey);
  ...
  private toRow(record: T): Record<string, string | number | null> {
    ...
    for (const column of this.getColumns()) {
      ...
      if (value === undefined && column.notnull && column.dflt_value === null) {
  ...
  private fromRow(row: Record<string, unknown>): T {
    ...
    for (const [name, value] of Object.entries(row)) {
      if (
        ...
        if (typeof value === 'string' && (value.startsWith('{') || value.startsWith('['))) {
          ...
          payload[camel(name)] = name === 'quick' ? Boolean(value) : value;
        }
      }
    }
    ...
    return {
      ...
      syncStatus: String(row.sync_status ?? row.status ?? 'SYNCED') as T['syncStatus'],
      updatedAt: Number(row.updated_at ?? (Date.parse(String(row.created_at ?? 0)) || 0)),
...
export class SqliteLocalDatabase implements LocalDatabase {
  ...
  async transaction<T>(_collections: readonly CollectionName[], work: () => Promise<T> | T) {
  ...
  async purgeDisallowed(allowed: ReadonlySet<CollectionName>) {
    ...
    this.sqlite.withTransactionSync(() => {
      for (const name of collectionNames) {
        const columns = this.sqlite.getAllSync<Column>(`PRAGMA table_info(${quote(name)})`);
        const names = new Set(columns.map((column) => column.name));
        if (!names.has('sync_status') || !names.has('tenant_id')) continue;
        this.sqlite.runSync(
          allowed.has(name)
          ...
          ...(allowed.has(name) ? [this.scope.tenantId, scopeKey] : [this.scope.tenantId]),
        );
      }
    });
  }
  ...
  async clear() {
    this.sqlite.withTransactionSync(() => {
      for (const name of collectionNames) this.sqlite.runSync(`DELETE FROM ${quote(name)}`);
    });
  }
