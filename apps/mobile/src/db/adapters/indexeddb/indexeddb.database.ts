...
class IndexedDbCollection<T extends LocalRecord> implements CollectionRepository<T> {
  ...
  subscribe(listener: () => void) {
    const subscription = liveQuery(() => this.table().where('scope').equals(this.scopeKey).count()).subscribe(
      {
        next: listener,
        error: listener,
      },
      ...
      async () => {
        for (const name of collectionNames) {
          ...
          const removable = rows
            .filter((row) => row.syncStatus === 'SYNCED' && (!allowed.has(name) || row.scope !== scopeKey))
            .map((row) => row.id);
