...
import { collectionNames, type CollectionName, type LocalRecord } from '../../types';
...
export class PosIndexedDb extends Dexie {
  constructor() {
    ...
    this.version(1).stores(Object.fromEntries(collectionNames.map((name) => [name, indexes])));
  }

  records<T extends LocalRecord = LocalRecord>(name: CollectionName): Table<T, string> {
