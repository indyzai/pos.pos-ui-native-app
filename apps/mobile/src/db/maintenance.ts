...
export async function clearLocalUiData(database?: LocalDatabase | null): Promise<void> {
  if (!hasNativeDatabase) {
    ...
    await Promise.all([
      ...
      database
        ? database.clear()
        : import('./adapters/indexeddb/indexeddb.client').then(({ clearIndexedDbLocalData }) =>
  ...
  sqlite.withTransactionSync(() => {
    for (const table of legacyTables) sqlite.runSync(`DELETE FROM "${table}"`);
  });
}
