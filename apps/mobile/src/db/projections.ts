...
export async function replaceLocalPayloads<T extends { id: string }>(
  ...
  const records: LocalRecord<T>[] = payloads.map((payload) => {
    const metadata = payload as T & { serverVersion?: number; updatedAt?: number | string; status?: string };
    const sourceStatus = String(metadata.status ?? '').toUpperCase();
    const syncStatus =
      sourceStatus === 'FAILED' ? 'FAILED' : sourceStatus.includes('PENDING') ? 'PENDING' : 'SYNCED';
    return {
...
export const payloadsFromRecords = <T>(records: readonly LocalRecord<T>[]): T[] =>
