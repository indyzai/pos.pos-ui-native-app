    ...
    scope.role === 'cashier'
      ...
      : scope.role === 'manager'
        ...
        : scope.role === 'admin'
          ? 'assigned-stores'
          : 'on-demand';
  return { role: scope.role, collections: new Set(roleCollections[scope.role]), detailMode };
}

export function assertCollectionAllowed(scope: DatabaseScope, collection: CollectionName): void {
  if (!getRoleDataManifest(scope).collections.has(collection)) {
    throw new Error(`${scope.role} cannot provision local collection ${collection}.`);
  }
}
