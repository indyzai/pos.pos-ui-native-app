      ...
      <View style={s.overlay}>
        ...
        <SafeAreaView edges={['bottom']} style={[s.sheet, { backgroundColor: c.surface }]}>
          ...
          <ScrollView contentContainerStyle={s.body}>
            <View style={[s.receipt, { backgroundColor: c.background, borderColor: c.outlineMuted }]}>
              ...
              {receipt.customer && (
                <Text style={[s.meta, { color: c.textSecondary }]}>Customer · {receipt.customer.name}</Text>
              )}
              {receipt.orderContext?.orderMode && (
                <Text style={[s.meta, { color: c.textSecondary }]}>
                  {orderModeLabel(receipt.orderContext.orderMode)}
                  {receipt.orderContext.tableName ? ` · ${receipt.orderContext.tableName}` : ''}
                </Text>
              )}
              ...
              {receipt.items.map((item) => (
                <View key={item.lineId || item.id} style={s.line}>
                  ...
                  <Text style={[s.lineValue, { color: c.text }]}>{money(item.price * item.quantity)}</Text>
                </View>
              ))}
              <View style={[s.divider, { backgroundColor: c.outlineMuted }]} />
              ...
              <View style={[s.totalLine, { borderTopColor: c.outlineMuted }]}>
                ...
                <Text style={[s.totalValue, { color: c.primary }]}>{money(receipt.totals.total)}</Text>
              </View>
              {receipt.payment.scrap && (
                <ReceiptLine label="Scrap exchange" value={`-${money(receipt.payment.scrap.total)}`} />
              )}
              <ReceiptLine
                label="Payment"
                value={receipt.payment.scrap ? `SCRAP + ${receipt.payment.method}` : receipt.payment.method}
              />
          ...
          <View style={s.actions}>
            {onPrint && (
              <AppPressable
                ...
                style={[
                  s.secondary,
                  { borderColor: c.outline, backgroundColor: c.background, opacity: printing ? 0.55 : 1 },
                ]}
              >
                <Printer size={17} color={c.text} />
                <Text style={[s.actionText, { color: c.text }]}>{printing ? 'Queuing…' : 'Print'}</Text>
              </AppPressable>
            )}
            <AppPressable
              onPress={() => void share()}
