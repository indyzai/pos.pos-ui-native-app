...
export function buildReportMetrics(
  ...
  const ids = new Set(
    sales.flatMap((sale) => [sale.id, sale.billId, sale.offlineId].filter((id): id is string => Boolean(id))),
  );
  const visibleRefunds = refunds.filter(
    (refund) =>
      new Date(refund.createdAt) >= start &&
      refund.status.toUpperCase() !== 'FAILED' &&
      (ids.has(refund.originalSaleId) ||
        Boolean(refund.originalInvoiceNumber && ids.has(refund.originalInvoiceNumber))),
  );
  ...
  return {
    ...
    refundTotal,
    taxTotal: sales.reduce((sum, sale) => sum + Number(sale.taxAmount || 0), 0),
