...
import { readWebPrintJobs, writeWebPrintJob } from '@indyzai/pos-database/web-client';
...
export async function savePrintJob(scope: string, job: LocalPrintJob): Promise<void> {
  ...
  getDatabase()
    ...
    .onConflictDoUpdate({ target: printJobs.id, set: { payload: JSON.stringify(job), status: job.status } })
    .run();
}
