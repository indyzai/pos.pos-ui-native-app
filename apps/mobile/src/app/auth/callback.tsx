...
export default function AuthCallbackRoute() {
  ...
  useEffect(() => {
    ...
    if (__DEV__) console.info('[Auth:pos-app] Callback route loaded', { hasCode: Boolean(code), hasState: Boolean(state), platform: Platform.OS });
    if (!code) {
    ...
    void (async () => {
      try {
        ...
        if (__DEV__) console.info('[Auth:pos-app] Session refreshed after callback', { needsDeviceSetup });
        if (Platform.OS === 'web') {
          if (__DEV__) console.info('[Auth:pos-app] Closing authentication popup');
          window.close();
          return;
        }
        router.replace(needsDeviceSetup ? '/device-setup' : '/billing');
      } catch (reason) {
        if (__DEV__) console.error('[Auth:pos-app] Callback failed', reason);
