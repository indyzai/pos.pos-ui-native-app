...
const CALENDAR_INTEGRATION_GUIDE_URL = 'https://docs.openpos.app/use/calendar-integration';
...
export function CalendarSettingsScreen() {
    ...
    const [isDeletingOpenPOSCalendar, setIsDeletingOpenPOSCalendar] = useState(false);
    ...
    const handleSelectCalendarPushColor = async (color: string) => {
        const updated = await updateOpenPOSCalendarColor(color);
        setCalendarPushColorState(color);
        await loadCalendarPushTargetState();
        showToast({
            title: tFallback(t, 'settings.calendarMobile.calendarColorUpdated', 'Calendar color updated'),
            message: updated
                ? tFallback(t, 'settings.calendarMobile.calendarColorUpdatedMessage', 'OpenPOS calendar color was updated.')
                : tFallback(t, 'settings.calendarMobile.calendarColorSavedMessage', 'OpenPOS will use this color when it creates the calendar.'),
            tone: 'success',
            durationMs: 3000,
        });
    };

    const performDeleteOpenPOSCalendar = useCallback(async () => {
        if (isDeletingOpenPOSCalendar) return;
        setIsDeletingOpenPOSCalendar(true);
        try {
            ...
            showToast({
                ...
                message: tr('settings.calendarMobile.theOpenPOSCalendarAndAllItsEventsHaveBeenRemoved'),
                tone: 'success',
                durationMs: 3500,
            });
        } catch (error) {
            ...
            setIsDeletingOpenPOSCalendar(false);
        }
    }, [isDeletingOpenPOSCalendar, loadCalendarPushTargetState, tr, showToast]);

    const handleDeleteOpenPOSCalendar = useCallback(() => {
        if (isDeletingOpenPOSCalendar) return;
        Alert.alert(
            tr('settings.calendarMobile.deleteOpenPOSCalendar'),
            ...
            [
                { text: t('common.cancel'), style: 'cancel' },
                {
                    ...
                    style: 'destructive',
                    onPress: () => {
                        void performDeleteOpenPOSCalendar();
                    },
                },
            ],
        );
    }, [isDeletingOpenPOSCalendar, performDeleteOpenPOSCalendar, t, tr]);
    ...
    const selectedSharedAccountCalendarForPush = Boolean(
        ...
        && !selectedCalendarPushTarget.isOpenPOSDedicated
    ...
    const selectedManagedOpenPOSCalendarForPush = calendarPushTargetCalendarId === null
        || selectedCalendarPushTarget?.isOpenPOSManaged === true;
    const hasDedicatedAccountCalendarForPush = calendarPushTargets.some((calendar) =>
        calendar.isOpenPOSDedicated && !calendar.isLocalOnly
    );
    const getCalendarPushTargetDescription = (calendar: CalendarPushTargetCalendar): string => {
        const kind = calendar.isOpenPOSDedicated
    ...
    const defaultLocalTargetOption = {
        ...
        name: tr('settings.calendarMobile.openposCalendar'),
    ...
    const calendarPushTargetOptions: Array<{
        ...
        color?: string;
            ...
            ...calendarPushTargets
                .filter((calendar) => {
                    if (calendar.isOpenPOSManaged && calendar.id !== calendarPushTargetCalendarId) return false;
                    if (
                        hasDedicatedAccountCalendarForPush
                        && calendar.isOpenPOSDedicated
                        && calendar.isLocalOnly
                ...
                <View style={[styles.settingCard, { backgroundColor: tc.cardBg, marginBottom: 16 }]}>
                    ...
                    {calendarPushOpen && calendarPushEnabled && calendarPushPermission === 'granted' && (
                        <View style={{ borderTopWidth: 1, borderTopColor: tc.border }}>
                            ...
                            {selectedManagedOpenPOSCalendarForPush && (
                                <View style={[styles.settingRowColumn, { borderTopWidth: 1, borderTopColor: tc.border }]}>
                                    <Text style={[styles.settingLabel, { color: tc.text }]}>
                                        {tFallback(t, 'settings.calendarMobile.openposCalendarColor', 'OpenPOS calendar color')}
                                    </Text>
                                    <Text style={[styles.settingDescription, { color: tc.secondaryText }]}>
                                        {tFallback(t, 'settings.calendarMobile.openposCalendarColorDesc', 'Applies to the OpenPOS-created calendar; shared account calendars keep their own color.')}
                                    </Text>
                                        ...
                                        {CALENDAR_PUSH_COLOR_OPTIONS.map((color) => {
                                            ...
                                            return (
                                                <TouchableOpacity
                                                    key={color}
                                                    accessibilityRole="button"
                                                    accessibilityLabel={`${tFallback(t, 'settings.calendarMobile.openposCalendarColor', 'OpenPOS calendar color')} ${color}`}
                                                    accessibilityState={{ selected }}
                            ...
                            <TouchableOpacity
                                accessibilityRole="button"
                                accessibilityLabel={tr('settings.calendarMobile.deleteOpenPOSCalendar')}
                                onPress={handleDeleteOpenPOSCalendar}
                                disabled={isDeletingOpenPOSCalendar}
                                style={[
                                    styles.settingRow,
                                    {
                                        borderTopWidth: 1,
                                        borderTopColor: tc.border,
                                        opacity: isDeletingOpenPOSCalendar ? 0.6 : 1,
                                    },
                                ]}
                            >
                                <View style={styles.settingInfo}>
