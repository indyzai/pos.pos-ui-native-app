...
describe('quick-capture utils', () => {
    ...
    it('builds capture file URIs inside the target directory', () => {
        expect(buildCaptureFileUri('file:///document/audio-captures', 'openpos-audio.m4a'))
            .toBe('file:///document/audio-captures/openpos-audio.m4a');
        expect(buildCaptureFileUri('file:///document/audio-captures/', 'openpos-audio.wav'))
            .toBe('file:///document/audio-captures/openpos-audio.wav');
    });

    it('selects the first existing capture file and skips missing candidates', () => {
        const missingDestination = {
            uri: 'file:///document/openpos-audio.m4a',
