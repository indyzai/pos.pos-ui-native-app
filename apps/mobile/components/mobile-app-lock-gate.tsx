  ...
  const authenticate = useCallback(async () => {
    ...
    try {
      const result = await authenticateWithDeviceLock({
        promptMessage: resolveText('appLock.prompt', 'Unlock OpenPOS'),
  ...
  const message = errorKey
    ? resolveText(errorKey, errorKey === 'appLock.unavailable'
      ...
      : errorKey === 'appLock.cancelled'
        ? 'OpenPOS is still locked.'
        : 'Authentication failed. Try again.')
    : resolveText('appLock.description', 'Use your device lock to open OpenPOS. This protects the app view, not the on-device database.');
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: tc.bg }]} edges={['top', 'right', 'bottom', 'left']}>
      <View style={styles.content}>
        ...
        <Text style={[styles.title, { color: tc.text }]}>{resolveText('appLock.title', 'OpenPOS is locked')}</Text>
