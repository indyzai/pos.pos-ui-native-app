...
export function useInventory() {
    ...
    useEffect(() => {
        ...
        void replaceLocalPayloads(local.database, 'products', query.data.cache.products).then(
            ...
            (reason) =>
                setProjectionError(
                    reason instanceof Error ? reason.message : 'Unable to update local stock.',
                ),
        );
    ...
    const jobs = useQuery({
        ...
        refetchInterval: (result) =>
            result.state.data?.some((job) => job.status === 'PENDING' || job.status === 'RUNNING')
                ? 1000
                : false,
    });
    ...
    return useMemo(
        () => ({
            ...
            error:
                local.error || auth.error || projectionError || (error instanceof Error ? error.message : ''),
