                              ...
                              <View style={s.cardBottom}>
                                  ...
                                  <AppPressable
                                      disabled={
                                          !order.items.some(
                                              (item) =>
                                                  refundableQuantity(order, item.productId, data.refunds) > 0,
                                          )
                                      }
                ...
                onConfirm={(selections, reason, method) => {
                    void data
                        ...
                        .then(() => {
                            ...
                            showSnackbar(
                                'Refund saved',
                                'The credit note is stored locally and will sync on refresh.',
                            );
                        })
                        .catch((error) =>
                            showSnackbar(
                                'Could not save refund',
...
function Tab({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
    ...
    return (
        <AppPressable
            onPress={onPress}
            style={[s.tab, { backgroundColor: active ? c.primary : c.surfaceMuted }]}
        >
