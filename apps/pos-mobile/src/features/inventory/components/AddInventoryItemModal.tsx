...
import type { CreateInventoryItemInput, InventoryProduct, ProductReferenceData } from '@indyzai/feature-inventory/types';
  ...
  useEffect(() => {
    ...
    void inventoryApi.loadProductReferences().then(
      ...
      (error) => {
        if (!active) return;
        setLoading(false);
        showSnackbar('Could not load product fields', error instanceof Error ? error.message : 'Try again.');
      },
    );
    return () => {
      active = false;
              ...
              <View style={s.row}>
                <View style={s.flex}>
                  <AppDropdown
                    label="Unit"
                    value={unitId}
                    options={references.units.map((x) => ({ value: x.id, label: `${x.name} (${x.code})` }))}
                    loading={loading}
