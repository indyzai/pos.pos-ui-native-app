...
export function StockReconciliationScreen() {
    ...
    const submit = async () => {
        if (!selected.length)
            return showSnackbar('Select products', 'Enter a count for at least one product.');
        if (hasInvalidCounts)
            return showSnackbar('Invalid count', 'Enter a valid count for every added product.');
        setSaving(true);
        try {
    ...
    const edit = (record: LocalRecord<StockReconciliationReport>) => {
        ...
        setCounts(
            Object.fromEntries(report.lines.map((line) => [line.productId, String(line.countedQuantity)])),
        );
        ...
        >
            ...
            <View style={[s.panel, { backgroundColor: c.surface, borderColor: c.outlineMuted }]}>
                ...
                <View style={s.actions}>
                    ...
                    <AppPressable
                        ...
                        style={[
                            s.primary,
                            { backgroundColor: c.primary, opacity: selected.length ? 1 : 0.5 },
                        ]}
                    >
                ...
                {visibleDrafts.map((record) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <AppPressable
                                onPress={() => edit(record)}
                                style={[s.edit, { borderColor: c.primary }]}
                            >
                    ...
                    {completedReports.map((record) => (
                        ...
                        >
                            <View style={s.productCopy}>
                                ...
                                <Text style={[s.productStock, { color: c.textSecondary }]}>
                                    {record.payload.lines.length} products · Loss ₹
                                    {record.payload.totalLossValue.toFixed(2)}
                                </Text>
                            </View>
                        </View>
                    ))}
            ...
            <BarcodeScannerModal
                ...
                onScan={(value) => {
                    ...
                    const product = products.data.find(
                        ({ payload }) =>
                            payload.barcode?.toLowerCase() === normalized ||
                            payload.sku?.toLowerCase() === normalized,
                    )?.payload;
