...
import { PharmacyPrescriptionContext } from '@indyzai/feature-billing/native/PharmacyPrescriptionContext';
    ...
    return (
        <BaseBillingLayout
            ...
            onBeforeCheckout={({ cart }) => {
                if (
                    cart.items.some(
                        (item) => item.details?.prescriptionRequired || item.details?.scheduledDrug,
                    ) &&
                    !doctorName.trim()
                ) {
                    showSnackbar(
                        'Prescription details required',
                        'Enter the prescribing doctor before checkout.',
                    );
                    return false;
                }
                return true;
                ...
                <PharmacyBatchDialog
                    ...
                    batches={
                        billing.data?.cache.productBatches.filter(
                            (batch) => batch.productId === pharmacyProduct?.id,
                        ) || []
                    }
