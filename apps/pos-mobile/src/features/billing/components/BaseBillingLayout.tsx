...
export function BaseBillingLayout({
    ...
    const { can } = usePermissions('pos');
    const managerAccess = can('manager.actions');
    ...
    const billingAllowed =
        can('billing.create') && (!counterRequired || !!auth.session?.organization?.activeSession);
    ...
    const createQuickProduct = async (input: Parameters<typeof inventoryApi.create>[0]) => {
        ...
        try {
            ...
            showSnackbar(
                'Product created',
                `${input.name} was added to this order as an incomplete product.`,
            );
        } catch (error) {
        ...
        () =>
            PanResponder.create({
                ...
                onPanResponderMove: (_event, gesture) =>
                    cartContentTranslateX.setValue(Math.max(0, gesture.dx)),
    ...
    const holdCurrentOrder = async () => {
        ...
        try {
            ...
            logger.error('Could not hold order', {
            ...
            <View style={s.workspace}>
                ...
                >
                    <CatalogToolbar
                        ...
                        onToggleStats={
                            isEnabled('advancedReporting')
                                ? () => setStatsVisible((value) => !value)
                                : undefined
                        }
                        searchPlaceholder={mode.searchPlaceholder}
                    />
                    {mode.mode !== 'retail' && <BillingModeBadge config={mode} />}
                    {headerSlot}
                ...
                {isWide && cartOpen && (
                    <View style={[s.rightCart, { borderLeftColor: themeColors.outline }]}>
                        <OrderCart
                            ...
                            allowItemDiscounts={isEnabled('allowItemDiscounts') && can('billing.discount')}
                            allowOrderDiscounts={isEnabled('allowOrderDiscounts') && can('billing.discount')}
                            heldOrderCount={heldOrders.orders.length}
                            onShowHeldOrders={() =>
                                openCartPage('held-orders', () => setHeldOrdersOpen(true))
                            }
            ...
            {isWide && !cartOpen && (
                <AppPressable onPress={() => setCartOpen(true)} style={s.wideCart}>
                    ...
                    <Text style={s.wideCartText}>
                        {cart.itemCount
                            ? `${cart.itemCount} · ${formatCurrency(cart.total, currencyCode, 0)}`
                            : 'Cart'}
                    </Text>
                </AppPressable>
            )}
                    ...
                    innerContent={
                        cartPage === 'cart' ? undefined : (
                            <Animated.View
                                ...
                                style={[
                                    s.cartInnerPage,
                                    { transform: [{ translateX: cartContentTranslateX }] },
                                ]}
                            >
                ...
                onPrint={
                    receipt && auth.session?.organization?.activeSession
                        ? async () => {
                              ...
                              try {
                                  const job = await printingApi.queueReceipt(
                                      receipt.id,
                                      session.counterId,
                                      session.branchId,
                                  );
