...
export function PurchasesScreen({
    ...
    const total = useMemo(
        () =>
            lines.reduce(
                (sum, line) => sum + line.quantity * line.purchasePrice * (1 + line.taxRate / 100),
                0,
            ),
        [lines],
    );
    ...
    useEffect(() => {
        if (!bottomNavigationHidden)
            setCenterItem({ label: 'New PO', icon: Plus, onPress: () => setOpen(true) });
    ...
    const save = async (status: Purchase['status']) => {
        ...
        if (!supplier.trim()) return showSnackbar('Supplier required', 'Enter a supplier name.');
        ...
        const payload: Purchase = {
            ...
            total,
            createdAt:
                purchases.data.find((row) => row.id === editingId)?.payload.createdAt ||
                            ...
                            >
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text style={{ color: c.textSecondary, fontSize: 11 }}>
                                        {payload.sku || payload.barcode || 'Existing product'} · ₹
                                        {payload.price}
                                    </Text>
                                </View>
