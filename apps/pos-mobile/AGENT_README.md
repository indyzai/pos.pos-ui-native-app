...
- Add focused Bun tests under `apps/pos-mobile/tests/features/<feature>/` for persistence, retry, and tenant isolation.
- Run `bun test apps/pos-mobile/tests`, `bunx tsc --noEmit -p apps/pos-mobile/tsconfig.json`, and `bun run format:check` before handing over.
