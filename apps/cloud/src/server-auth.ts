...
export function resolveAllowedAuthTokensFromEnv(env: Record<string, string | undefined>): AllowedAuthTokens | null {
    const values = [
        env.OPEN_POS_CLOUD_AUTH_TOKENS,
        readOptionalEnvFile(env, 'OPEN_POS_CLOUD_AUTH_TOKENS_FILE'),
        env.OPEN_POS_CLOUD_TOKEN,
        readOptionalEnvFile(env, 'OPEN_POS_CLOUD_TOKEN_FILE'),
    ]
        .map((value) => String(value || '').trim())
        .filter((value) => value.length > 0);
    if (values.length === 0) {
        if (parseBoolEnv(env.OPEN_POS_CLOUD_ALLOW_ANY_TOKEN)) {
            logWarn('OPEN_POS_CLOUD_ALLOW_ANY_TOKEN is enabled. Prefer OPEN_POS_CLOUD_AUTH_TOKENS for stronger access control.');
            return null;
        }
        throw new Error(
            'Cloud auth is not configured. Set OPEN_POS_CLOUD_AUTH_TOKENS (or legacy OPEN_POS_CLOUD_TOKEN), or explicitly set OPEN_POS_CLOUD_ALLOW_ANY_TOKEN=true to enable token namespace mode.'
        );
    }
