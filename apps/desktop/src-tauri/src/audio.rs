...
fn validate_managed_audio_path(data_dir: &Path, audio_path: &Path) -> Result<PathBuf, String> {
    ...
    if !audio_file.starts_with(&capture_dir) {
        return Err("Audio file must be a managed OpenPOS capture".into());
    }
    Ok(audio_file)
}
...
fn validate_managed_whisper_model_path(
    ...
    Err("Whisper model must be installed by OpenPOS".into())
}
...
fn validate_managed_parakeet_model_dir(
    ...
    if model_dir != expected_dir {
        return Err("Parakeet model must be installed by OpenPOS".into());
    }
    Ok(model_dir)
}
...
fn audio_capture_file_name(now: SystemTime) -> Result<String, String> {
    let timestamp = now
        ...
        .as_nanos();
    Ok(format!("openpos-audio-{timestamp}.wav"))
}
...
mod tests {
    ...
    fn parakeet_model_dir_uses_app_data_install_folder() {
        let data_dir = Path::new("/home/dd/.local/share/openpos");

        assert_eq!(
            parakeet_model_dir(data_dir),
            PathBuf::from("/home/dd/.local/share/openpos/parakeet-model")
        );
    }
    ...
    fn audio_capture_file_name_uses_subsecond_precision() {
        ...
        assert_eq!(first, "openpos-audio-1000000000.wav");
        assert_eq!(second, "openpos-audio-1000001000.wav");
    }
    ...
    fn managed_audio_path_validation_rejects_outside_capture_dir() {
        ...
        assert!(error.contains("managed OpenPOS capture"));
    }
    ...
    fn managed_model_path_validation_rejects_outside_paths() {
        ...
        assert!(whisper_error.contains("installed by OpenPOS"));
        ...
        assert!(parakeet_error.contains("installed by OpenPOS"));
    }
