...
static NSNumber *ck_retry_after_seconds(NSError *error) {
  ...
  if ([partial isKindOfClass:[NSDictionary class]]) {
    ...
    for (id value in [(NSDictionary *)partial allValues]) {
      ...
      if (nested &&
          (!longest || [nested doubleValue] > [longest doubleValue])) {
        longest = nested;
      }
    }
    return longest;
...
static BOOL ck_error_is_only_unknown_item(NSError *error) {
  ...
  if (![partials isKindOfClass:[NSDictionary class]] || partials.count == 0)
    return NO;
...
static char *ck_success_json(void) { return strdup("{\"ok\":true}"); }
...
static const MWFieldSpec kTaskFields[] = {
    ...
    {"suppressOpenPOSReminders", "suppressOpenPOSReminders", MWFieldKindBool},
    ...
    {"statusBeforeProjectArchive", "statusBeforeProjectArchive",
     MWFieldKindString},
    {"completedAtBeforeProjectArchive", "completedAtBeforeProjectArchive",
     MWFieldKindDate},
    {"isFocusedTodayBeforeProjectArchive", "isFocusedTodayBeforeProjectArchive",
     MWFieldKindBool},
...
static const size_t kTaskFieldsCount =
    sizeof(kTaskFields) / sizeof(kTaskFields[0]);
...
static const MWFieldSpec kProjectFields[] = {
    ...
    {"startDate", "startDateText",
     MWFieldKindDate}, // Production "startDate" is a Date field; strings never
                       // fit it
};
static const size_t kProjectFieldsCount =
    sizeof(kProjectFields) / sizeof(kProjectFields[0]);
...
static const MWFieldSpec kSectionFields[] = {
    ...
    {"updatedAt", "updatedAt", MWFieldKindDate},
    {"deletedAt", "deletedAt", MWFieldKindDate},
    {"deletedAtBeforeProjectArchive", "deletedAtBeforeProjectArchive",
     MWFieldKindDate},
    {"projectArchivedAt", "projectArchivedAt", MWFieldKindDate},
};
static const size_t kSectionFieldsCount =
    sizeof(kSectionFields) / sizeof(kSectionFields[0]);
...
static const size_t kAreaFieldsCount =
    sizeof(kAreaFields) / sizeof(kAreaFields[0]);
...
static const size_t kPersonFieldsCount =
    sizeof(kPersonFields) / sizeof(kPersonFields[0]);

static const MWFieldSpec kSettingsFields[] = {
    {"payload", "payload", MWFieldKindJsonString},
    {"updatedAt", "updatedAt", MWFieldKindDate},
};
static const size_t kSettingsFieldsCount =
    sizeof(kSettingsFields) / sizeof(kSettingsFields[0]);

static void ck_get_field_specs(NSString *recordType,
                               const MWFieldSpec **outSpecs, size_t *outCount) {
  if ([recordType isEqualToString:@"OpenPOSTask"]) {
    *outSpecs = kTaskFields;
    *outCount = kTaskFieldsCount;
  } else if ([recordType isEqualToString:@"OpenPOSProject"]) {
    *outSpecs = kProjectFields;
    *outCount = kProjectFieldsCount;
  } else if ([recordType isEqualToString:@"OpenPOSSection"]) {
    *outSpecs = kSectionFields;
    *outCount = kSectionFieldsCount;
  } else if ([recordType isEqualToString:@"OpenPOSArea"]) {
    *outSpecs = kAreaFields;
    *outCount = kAreaFieldsCount;
  } else if ([recordType isEqualToString:@"OpenPOSPerson"]) {
    *outSpecs = kPersonFields;
    *outCount = kPersonFieldsCount;
  } else if ([recordType isEqualToString:@"OpenPOSSettings"]) {
    *outSpecs = kSettingsFields;
    *outCount = kSettingsFieldsCount;
  } else {
    *outSpecs = NULL;
    *outCount = 0;
  }
...
static void ck_apply_fields(NSDictionary *json, CKRecord *record,
                            NSString *recordType) {
  const MWFieldSpec *specs;
  size_t count;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    case MWFieldKindInt:
      if ([value isKindOfClass:[NSNumber class]])
        record[ckKey] = @([value longLongValue]);
      break;
    case MWFieldKindBool:
      if ([value isKindOfClass:[NSNumber class]])
        record[ckKey] = @([value boolValue] ? 1LL : 0LL);
      break;
...
static NSDictionary *ck_json_from_record(CKRecord *record) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    case MWFieldKindBool:
      ...
      if ([value isKindOfClass:[NSNumber class]])
        result[jsKey] = ([value longLongValue] == 1) ? @YES : @NO;
      break;
    case MWFieldKindStringArray:
      if ([value isKindOfClass:[NSArray class]])
        result[jsKey] = value;
...
#if defined(OPEN_POS_NATIVE_MAPPER_FIXTURE_CHECK)
...
char *openpos_cloudkit_round_trip_task_json(const char *json_utf8) {
  @autoreleasepool {
    if (!json_utf8)
      return NULL;
    ...
    CKRecordZoneID *zoneID =
        [[CKRecordZoneID alloc] initWithZoneName:@"OpenPOSFixtureZone"
                                       ownerName:CKCurrentUserDefaultName];
    CKRecordID *recordID = [[CKRecordID alloc] initWithRecordName:taskID
                                                           zoneID:zoneID];
    CKRecord *record = [[CKRecord alloc] initWithRecordType:@"OpenPOSTask"
                                                   recordID:recordID];
    ck_apply_fields(json, record, @"OpenPOSTask");
...
static NSString *ck_serialize_token(CKServerChangeToken *token) {
  ...
  NSData *data = [NSKeyedArchiver archivedDataWithRootObject:token
                                       requiringSecureCoding:YES
                                                       error:&err];
  if (!data || err) {
    NSLog(@"[OpenPOS] Failed to serialize change token: %@", err);
    return nil;
  }
...
static CKServerChangeToken *ck_deserialize_token(NSString *base64) {
  ...
  CKServerChangeToken *token =
      [NSKeyedUnarchiver unarchivedObjectOfClass:[CKServerChangeToken class]
                                        fromData:data
                                           error:&err];
  if (err) {
    NSLog(@"[OpenPOS] Failed to deserialize change token: %@", err);
    return nil;
  }
  return token;
}
...
char *openpos_cloudkit_account_status(void) {
  @autoreleasepool {
    openpos_ck_ensure_container();

    __block CKAccountStatus status = CKAccountStatusCouldNotDetermine;
    __block NSError *opError = nil;
    dispatch_semaphore_t sem = dispatch_semaphore_create(0);

    [_ckContainer accountStatusWithCompletionHandler:^(CKAccountStatus s,
                                                       NSError *error) {
      status = s;
      opError = error;
    ...
    long waited = dispatch_semaphore_wait(
        sem, dispatch_time(DISPATCH_TIME_NOW, kTimeoutSec * NSEC_PER_SEC));
    if (waited != 0)
      return ck_copy_json(@{@"status" : @"timeout"});
    if (opError)
      return ck_error_json(opError);
...
char *openpos_cloudkit_ensure_zone(void) {
  @autoreleasepool {
    openpos_ck_ensure_container();
    ...
    op.modifyRecordZonesCompletionBlock =
        ^(NSArray *saved __unused, NSArray *deleted __unused, NSError *error) {
          opError = error;
          dispatch_semaphore_signal(sem);
        };
...
char *openpos_cloudkit_ensure_subscription(void) {
  @autoreleasepool {
    openpos_ck_ensure_container();
    ...
    [_ckPrivateDB fetchSubscriptionWithID:kSubscriptionID
                        completionHandler:^(CKSubscription *sub,
    ...
    dispatch_semaphore_wait(
        fetchSem, dispatch_time(DISPATCH_TIME_NOW, kTimeoutSec * NSEC_PER_SEC));
    if (alreadyExists)
      return ck_success_json();
    ...
    CKModifySubscriptionsOperation *op = [[CKModifySubscriptionsOperation alloc]
        initWithSubscriptionsToSave:@[ sub ]
            subscriptionIDsToDelete:nil];
    op.qualityOfService = NSQualityOfServiceUtility;

    __block NSError *opError = nil;
    dispatch_semaphore_t sem = dispatch_semaphore_create(0);
    op.modifySubscriptionsCompletionBlock =
        ^(NSArray *saved __unused, NSArray *deleted __unused, NSError *error) {
          opError = error;
          dispatch_semaphore_signal(sem);
        };
    ...
    long waited = dispatch_semaphore_wait(
        sem, dispatch_time(DISPATCH_TIME_NOW, kTimeoutSec * NSEC_PER_SEC));
    if (waited != 0)
      return ck_copy_json(@{@"error" : @"subscription-timeout"});
    if (opError)
      return ck_error_json(opError);
    return ck_success_json();
  }
...
char *openpos_cloudkit_fetch_all_records(const char *record_type_cstr) {
  @autoreleasepool {
    if (!record_type_cstr)
      return strdup("[]");
    openpos_ck_ensure_container();
    ...
    do {
      ...
      if (batchError) {
        ...
        // The type is auto-created on first save in the Development
        // environment. CKErrorUnknownItem: record type unknown to the client
        // framework. CKErrorServerRejectedRequest: server rejects the query
        // because the
...
char *openpos_cloudkit_fetch_changes(const char *change_token_base64_cstr) {
  @autoreleasepool {
    openpos_ck_ensure_container();

    NSString *tokenB64 =
        change_token_base64_cstr
    ...
    __block NSMutableDictionary<NSString *, NSMutableArray *> *deletedByType =
        [NSMutableDictionary dictionary];
    ...
    op.recordWithIDWasDeletedBlock =
        ^(CKRecordID *recordID, NSString *recordType) {
          dispatch_sync(cbQueue, ^{
            if (!deletedByType[recordType])
              deletedByType[recordType] = [NSMutableArray array];
            [deletedByType[recordType] addObject:recordID.recordName];
          });
        };

    op.recordZoneFetchCompletionBlock = ^(
        CKRecordZoneID *zoneID __unused, CKServerChangeToken *serverChangeToken,
        NSData *clientTokenData __unused, BOOL moreComing __unused,
        NSError *error) {
      dispatch_sync(cbQueue, ^{
        if (error) {
    ...
    if (tokenExpired)
      return ck_copy_json(
          @{@"tokenExpired" : @YES,
            ...
            @"deletedIDs" : @{}});
    if (zoneError)
      return ck_error_json(zoneError);
    ...
    if (newTokenB64)
      result[@"changeToken"] = newTokenB64;

    return ck_copy_json(result);
  }
}

/// Fetch records by their IDs. Returns a mutable dictionary keyed by
/// CKRecordID. On error, returns nil and sets *outError. unknownItem (record
/// doesn't exist) is silently skipped — mirrors iOS fetchRecordsByID behaviour.
static NSMutableDictionary<CKRecordID *, CKRecord *> *
ck_fetch_records_by_id(NSArray<CKRecordID *> *ids, NSError **outError) {
  NSMutableDictionary<CKRecordID *, CKRecord *> *results =
      [NSMutableDictionary dictionary];
  if (ids.count == 0)
    return results;

  CKFetchRecordsOperation *op =
      [[CKFetchRecordsOperation alloc] initWithRecordIDs:ids];
  op.qualityOfService = NSQualityOfServiceUserInitiated;

  __block NSMutableArray<NSError *> *perRecordErrors = [NSMutableArray array];
  dispatch_semaphore_t sem = dispatch_semaphore_create(0);

  op.fetchRecordsCompletionBlock =
      ^(NSDictionary<CKRecordID *, CKRecord *> *recordsByID, NSError *error) {
        ...
        if (error) {
          if ([error.domain isEqualToString:CKErrorDomain] &&
            ...
            // Inspect per-record errors — suppress unknownItem, collect real
            // errors.
            NSDictionary *partials = error.userInfo[CKPartialErrorsByItemIDKey];
  ...
  if (waited != 0) {
    if (outError)
      *outError =
          [NSError errorWithDomain:CKErrorDomain
                              code:CKErrorNetworkFailure
                          userInfo:@{
                            NSLocalizedDescriptionKey : @"fetch-by-id-timeout"
                          }];
    return nil;
  }
  if (perRecordErrors.count > 0) {
    if (outError)
      *outError = perRecordErrors.firstObject;
    return nil;
  }
  return results;
}

static NSString *const kOpenPOSAttachmentRecordType = @"OpenPOSAttachment";
static NSString *const kOpenPOSAttachmentAssetField = @"asset";
...
static void ck_apply_attachment_metadata(NSDictionary *metadata,
                                         CKRecord *record) {
  NSArray<NSString *> *stringFields = @[
    @"attachmentId",
    @"ownerType",
...
static char *ck_save_single_record(CKRecord *record, NSString *timeoutName) {
  ...
  op.modifyRecordsCompletionBlock =
      ^(NSArray<CKRecord *> *saved __unused,
        NSArray<CKRecordID *> *deleted __unused, NSError *error) {
        saveError = error;
        dispatch_semaphore_signal(sem);
      };
  [_ckPrivateDB addOperation:op];
  long waited = dispatch_semaphore_wait(
      sem, dispatch_time(DISPATCH_TIME_NOW, kTimeoutSec * NSEC_PER_SEC));
  if (waited != 0)
    return ck_copy_json(@{@"error" : timeoutName ?: @"save-timeout"});
  if (saveError)
    return ck_error_json(saveError);
  return NULL;
}

char *openpos_cloudkit_save_attachment_asset(const char *record_name_cstr,
  ...
  @autoreleasepool {
    ...
    NSData *jsonData = [[NSString stringWithUTF8String:metadata_json_cstr]
    ...
    if (!fileURL)
      return ck_copy_json(@{@"error" : @"invalid-attachment-file"});
    ...
    if (!fetched && fetchError) {
      return ck_copy_json(@{
        @"error" : fetchError.localizedDescription
            ?: @"fetch-existing-attachment-failed"
      });
    }
    CKRecord *record =
        fetched[recordID]
            ?: [[CKRecord alloc] initWithRecordType:kOpenPOSAttachmentRecordType
                                           recordID:recordID];
    ck_apply_attachment_metadata(metadata, record);
    record[kOpenPOSAttachmentAssetField] =
        [[CKAsset alloc] initWithFileURL:fileURL];
...
char *openpos_cloudkit_fetch_attachment_asset(const char *record_name_cstr,
  ...
  @autoreleasepool {
    ...
    if (!fetched) {
      ...
      return ck_copy_json(@{
        @"error" : fetchError.localizedDescription ?: @"fetch-attachment-failed"
      });
    }
    ...
    CKAsset *asset = record[kOpenPOSAttachmentAssetField];
    ...
    if (!targetURL)
      return ck_copy_json(@{@"error" : @"invalid-attachment-target"});
    ...
    if (fileError)
      return ck_copy_json(@{
        @"error" : fileError.localizedDescription
            ?: @"attachment-target-directory-failed"
      });
    ...
    if (![fm copyItemAtURL:asset.fileURL toURL:targetURL error:&fileError]) {
      return ck_copy_json(@{
        @"error" : fileError.localizedDescription ?: @"attachment-copy-failed"
      });
    }

    NSMutableDictionary *metadata =
        [ck_attachment_metadata_from_record(record) mutableCopy];
    metadata[@"filePath"] = targetURL.path;
    return ck_copy_json(metadata);
  }
}

char *openpos_cloudkit_save_records(const char *record_type_cstr,
                                    const char *records_json_cstr) {
  @autoreleasepool {
    ...
    NSString *recordType = [NSString stringWithUTF8String:record_type_cstr];
    NSData *jsonData = [[NSString stringWithUTF8String:records_json_cstr]
        dataUsingEncoding:NSUTF8StringEncoding];
    if (!jsonData)
      return ck_copy_json(@{@"error" : @"invalid-json-input"});

    NSArray<NSDictionary *> *jsonRecords =
        [NSJSONSerialization JSONObjectWithData:jsonData options:0 error:nil];
    if (!jsonRecords || ![jsonRecords isKindOfClass:[NSArray class]] ||
        jsonRecords.count == 0) {
      return ck_copy_json(@{@"conflictIDs" : @[]});
    }
    ...
    for (NSDictionary *json in jsonRecords) {
      ...
      if ([rid isKindOfClass:[NSString class]] && rid.length > 0) {
        [recordIDs addObject:[[CKRecordID alloc] initWithRecordName:rid
                                                             zoneID:_ckZoneID]];
      }
    }
    ...
    NSMutableDictionary<CKRecordID *, CKRecord *> *existingByID =
        [NSMutableDictionary dictionary];
    for (NSUInteger i = 0; i < recordIDs.count; i += kBatchSize) {
      ...
      if (!fetched) {
        return ck_copy_json(@{
          @"error" : fetchError.localizedDescription ?: @"fetch-existing-failed"
        });
      }
    ...
    for (NSDictionary *json in jsonRecords) {
      ...
      CKRecordID *recordID = [[CKRecordID alloc] initWithRecordName:rid
                                                             zoneID:_ckZoneID];
    ...
    for (NSUInteger i = 0; i < recordsToSave.count; i += kBatchSize) {
      ...
      NSArray<CKRecord *> *batch =
          [recordsToSave subarrayWithRange:NSMakeRange(i, end - i)];

      CKModifyRecordsOperation *saveOp =
          [[CKModifyRecordsOperation alloc] initWithRecordsToSave:batch
                                                recordIDsToDelete:nil];
      saveOp.savePolicy = CKRecordSaveChangedKeys;
      saveOp.qualityOfService = NSQualityOfServiceUserInitiated;

      __block NSMutableArray<NSString *> *batchConflicts =
          [NSMutableArray array];
      __block NSMutableArray<NSError *> *batchErrors = [NSMutableArray array];
      dispatch_semaphore_t sem = dispatch_semaphore_create(0);

      saveOp.modifyRecordsCompletionBlock =
          ^(NSArray<CKRecord *> *saved __unused,
            NSArray<CKRecordID *> *deleted __unused, NSError *error) {
            if (error) {
              if ([error.domain isEqualToString:CKErrorDomain] &&
                ...
                NSDictionary *partialErrors =
                    error.userInfo[CKPartialErrorsByItemIDKey];
    ...
    if (nonConflictErrors.count > 0) {
      NSMutableDictionary *result = [NSMutableDictionary dictionary];
      result[@"error"] = [[nonConflictErrors firstObject] localizedDescription]
                             ?: @"save-failed";
...
char *openpos_cloudkit_delete_records(const char *record_type_cstr __unused,
                                      const char *record_ids_json_cstr) {
  @autoreleasepool {
    if (!record_ids_json_cstr)
      return ck_success_json();
    openpos_ck_ensure_container();

    NSData *jsonData = [[NSString stringWithUTF8String:record_ids_json_cstr]
        dataUsingEncoding:NSUTF8StringEncoding];
    if (!jsonData)
      return ck_success_json();
    NSArray<NSString *> *ids = [NSJSONSerialization JSONObjectWithData:jsonData
                                                               options:0
                                                                 error:nil];
    if (!ids || ![ids isKindOfClass:[NSArray class]] || ids.count == 0)
      return ck_success_json();

    NSMutableArray<CKRecordID *> *ckIDs =
        [NSMutableArray arrayWithCapacity:ids.count];
    for (NSString *rid in ids) {
      if ([rid isKindOfClass:[NSString class]] && rid.length > 0) {
        [ckIDs addObject:[[CKRecordID alloc] initWithRecordName:rid
                                                         zoneID:_ckZoneID]];
      }
    }
    ...
    for (NSUInteger i = 0; i < ckIDs.count; i += kBatchSize) {
      ...
      NSArray<CKRecordID *> *batch =
          [ckIDs subarrayWithRange:NSMakeRange(i, end - i)];
      ...
      long waited = dispatch_semaphore_wait(
          sem, dispatch_time(DISPATCH_TIME_NOW, kTimeoutSec * NSEC_PER_SEC));
      if (waited != 0)
        return ck_copy_json(@{@"error" : @"delete-timeout"});
      if (opError)
        return ck_error_json(opError);
    }
...
void openpos_cloudkit_register_for_remote_notifications(void) {
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSApplication sharedApplication] registerForRemoteNotifications];
  });
}

void openpos_cloudkit_set_pending_remote_change(void) {
  __c11_atomic_store(&_pendingRemoteChange, 1, __ATOMIC_SEQ_CST);
}

int openpos_cloudkit_consume_pending_remote_change(void) {
  return __c11_atomic_exchange(&_pendingRemoteChange, 0, __ATOMIC_SEQ_CST);
}

void openpos_cloudkit_free_string(char *ptr) {
  if (ptr)
    free(ptr);
}
