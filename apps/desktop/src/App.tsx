...
import { useStartupPromptQueue, type StartupPromptDescriptor } from '@openpos/core';
...
const DESKTOP_ONBOARDING_STORAGE_KEY = 'openpos:desktop:first-run-onboarding:v1';
...
const getDesktopReviewTarget = (installSource: InstallSource | null): { label: string; url: string } | null => {
    ...
    if (installSource === 'microsoft-store' || platform === 'windows') {
        return { label: 'Rate OpenPOS', url: MS_STORE_REVIEW_URL };
    }
    if (installSource === 'mac-app-store' || platform === 'macos') {
        return { label: 'Rate OpenPOS', url: MAC_APP_STORE_REVIEW_URL };
    }
    return {
        label: 'Open GitHub',
...
function App() {
    ...
    const trayTooltip = useMemo(() => buildTrayTooltip({
        appName: translateWithFallback(t, 'app.name', 'OpenPOS'),
                ...
                .then(async ({ getCurrentWindow }) => {
                    ...
                    const unlisten = await window.onCloseRequested(async (event) => {
                        ...
                        const closeAnyway = await requestConfirmation({
                            title: translateOrFallbackRef.current('app.closeStillSavingTitle', 'OpenPOS is still saving'),
                            description: translateOrFallbackRef.current(
                                'app.closeStillSavingBody',
                                'OpenPOS has not finished saving your recent changes. '
            ...
            <KeybindingProvider currentView={currentView} onNavigate={handleViewChange}>
                <Layout currentView={currentView} onViewChange={handleViewChange} onOpenSyncSettings={openSyncSettings}>
                    ...
                    <CloseBehaviorModal
                        ...
                        title={translateOrFallback('settings.closeBehaviorPromptTitle', 'Close OpenPOS?')}
                        description={translateOrFallback(
                            'settings.closeBehaviorPromptBody',
                            'Do you want OpenPOS to stay running in the tray or quit completely?'
                        )}
