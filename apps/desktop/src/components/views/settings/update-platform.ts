...
export const buildLinuxPostDownloadNotice = ({
  ...
  if (linuxFlavor === "arch") {
    if (installSource === "aur-source") {
      return `${downloadAURHint}: yay -Syu openpos / paru -Syu openpos`;
    }
    if (installSource === "aur-bin") {
      return `${downloadAURHint}: yay -Syu openpos-bin / paru -Syu openpos-bin`;
    }
    return `${downloadAURHint}: yay -Syu openpos / paru -Syu openpos`;
  }
  if (linuxFlavor === "debian") {
    return `${linuxUpdateHint} APT repo update: sudo apt update && sudo apt install --only-upgrade openpos. Local file install: sudo apt install ./<downloaded-file>.deb`;
  }
  if (linuxFlavor === "rpm") {
    return `${linuxUpdateHint} Repo update: sudo dnf upgrade openpos. Local file install: sudo dnf install ./<downloaded-file>.rpm`;
  }
