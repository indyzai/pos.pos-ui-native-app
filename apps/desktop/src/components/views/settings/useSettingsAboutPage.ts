...
const UPDATE_BADGE_AVAILABLE_KEY = 'openpos-update-available';
const UPDATE_BADGE_LAST_CHECK_KEY = 'openpos-update-last-check';
const UPDATE_BADGE_LATEST_KEY = 'openpos-update-latest';
...
export function useSettingsAboutPage({
    ...
    const handleCheckUpdates = useCallback(async () => {
        ...
        try {
            ...
            if (info.platform === 'linux' && linuxFlavor === 'arch') {
                ...
                setDownloadNotice(
                    'Recommended on macOS: brew update && brew upgrade --cask openpos',
                );
            } else {
    ...
    const handleDownloadUpdate = useCallback(async () => {
        ...
        if (installSource === 'homebrew') {
            ...
            setDownloadNotice(
                'Update via Homebrew: brew update && brew upgrade --cask openpos',
            );
            return;
        }
        ...
        if (installSource === 'scoop') {
            setDownloadNotice('Update via Scoop: scoop update openpos');
            return;
        }
        if (installSource === 'chocolatey') {
            await openLink(CHOCOLATEY_PACKAGE_URL);
            setDownloadNotice('Update via Chocolatey: choco upgrade openpos');
            return;
        }
