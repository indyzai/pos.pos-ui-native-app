...
import { DEFAULT_TASKNOTES_FOLDER, safeFormatDate, translateWithFallback, useTaskStore, type ObsidianTask, type Task } from '@openpos/core';
...
const OBSIDIAN_VIEW_STATE_STORAGE_KEY = 'openpos:view:obsidian:v1';
...
const buildOpenPOSTaskInitialProps = (
    task: ObsidianTask,
    sourceUri: string,
    sourceTitle: string,
    ...
    const status = task.completed ? 'done' : (task.taskNotesData?.openposStatus ?? 'inbox');
    const attachment = {
...
export function ObsidianView() {
    ...
    const openposTasks = useTaskStore((state) => state._allTasks);
    ...
    const [pendingOpenPOSTaskIds, setPendingOpenPOSTaskIds] = useState<Record<string, true>>({});
    ...
    const handleBringIntoOpenPOS = useCallback(async (task: typeof tasks[number]) => {
        ...
        if (openposTasks.some((existingTask) => hasObsidianSourceAttachment(
            existingTask,
            sourceIdentity,
            sourceUri,
            task.text,
        ))) {
            showToast(resolveText('obsidian.bringIntoOpenPOSAlreadyExists', 'Task already exists in OpenPOS.'), 'info');
            return;
        }
        setPendingOpenPOSTaskIds((current) => ({ ...current, [task.id]: true }));
        try {
            const result = await addTask(
                task.text,
                buildOpenPOSTaskInitialProps(
                    task,
                    sourceUri,
            ...
            if (!result.success) {
                throw new Error(result.error || resolveText('obsidian.bringIntoOpenPOSFailed', 'Could not add the task to OpenPOS.'));
            }
            showToast(resolveText('obsidian.bringIntoOpenPOSSuccess', 'Task added to OpenPOS.'), 'success');
        } catch (bringError) {
            showToast(
                bringError instanceof Error && bringError.message.trim()
                    ? bringError.message
                    : resolveText('obsidian.bringIntoOpenPOSFailed', 'Could not add the task to OpenPOS.'),
                'error',
                5000
            );
        } finally {
            setPendingOpenPOSTaskIds((current) => {
                const next = { ...current };
                delete next[task.id];
                return next;
            });
        }
    }, [addTask, openposTasks, resolveText, showToast, tasks]);
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
                                    {resolveText(
                                        ...
                                        'Import tasks from your Obsidian vault, keep the source note visible, and bring real commitments into OpenPOS when needed.'
                                    )}
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm text-foreground/85">
                                    {resolveText(
                                        ...
                                        'OpenPOS detected TaskNotes-style frontmatter in these files, so inline checklist tasks from other notes are ignored.'
                                    )}
                                </p>
                ...
                {canScan && tasks.length === 0 && hasCompletedScan && !isScanning && (
                    <section className="rounded-2xl border border-border bg-card p-8">
                        ...
                        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                            {resolveText(
                                'obsidian.emptyBody',
                                'OpenPOS scanned the configured folders but did not find any Markdown checklist items yet.'
                            )}
                        </p>
                    </section>
                )}
                ...
                {visibleTasks.length > 0 && (
                    <section className="space-y-3">
                        {visibleTasks.map((task) => {
                            ...
                            const isBringingIntoOpenPOS = Boolean(pendingOpenPOSTaskIds[task.id]);
                                ...
                                >
                                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                        ...
                                        <div className="flex flex-wrap items-center justify-end gap-2">
                                            <button
                                                type="button"
                                                onClick={() => void handleBringIntoOpenPOS(task)}
                                                disabled={isBringingIntoOpenPOS}
                                            ...
                                            >
                                                ...
                                                {isBringingIntoOpenPOS
                                                    ? resolveText('common.saving', 'Saving...')
                                                    : resolveText('obsidian.bringIntoOpenPOS', 'Bring into OpenPOS')}
                                            </button>
                                            <button
                                                type="button"
