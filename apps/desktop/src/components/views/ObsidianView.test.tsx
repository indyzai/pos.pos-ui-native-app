...
describe('ObsidianView', () => {
    ...
    it('brings an imported Obsidian task into OpenPOS with a source backlink', async () => {
        ...
        fireEvent.click(getByRole('button', { name: 'Bring into OpenPOS' }));
    ...
    it('keeps identical checkboxes distinct and recognizes one after its line number moves', async () => {
        ...
        const promoteButtons = getAllByRole('button', { name: 'Bring into OpenPOS' });
        ...
        fireEvent.click(getAllByRole('button', { name: 'Bring into OpenPOS' })[0]);
        await waitFor(() => {
            expect(useTaskStore.getState()._allTasks.filter((task) => task.title === 'Draft launch brief')).toHaveLength(2);
        });
    });
