...
describe('text-size', () => {
    beforeEach(() => {
        ...
        document.documentElement.style.removeProperty('--openpos-text-scale');
    });
    ...
    it('applies the selected preset to the document root', () => {
        ...
        expect(document.documentElement.style.getPropertyValue('--openpos-text-scale')).toBe('1.25');
    });
    ...
    it('applies the small preset below the default scale', () => {
        ...
        expect(document.documentElement.style.getPropertyValue('--openpos-text-scale')).toBe('0.875');
    });
});
