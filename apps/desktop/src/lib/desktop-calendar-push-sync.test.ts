...
import { useTaskStore, type CalendarSyncEntry, type Project, type Section, type Task } from '@openpos/core';
...
describe('desktop calendar push sync', () => {
    ...
    let ensureOpenPOSCalendar: ReturnType<typeof vi.fn<() => Promise<SystemCalendarPushTarget | null>>>;
    ...
    beforeEach(() => {
        ...
        ensureOpenPOSCalendar = vi.fn(async () => managedTarget);
    ...
    it('creates all-day events for due-date tasks in the managed OpenPOS calendar', async () => {
        setStoreTasks([makeTask({ dueDate: '2026-01-10' })]);

        await runFullDesktopCalendarPushSync();

        expect(ensureOpenPOSCalendar).toHaveBeenCalledTimes(1);
        expect(createEvent).toHaveBeenCalledWith(expect.objectContaining({
            calendarId: 'cal-openpos',
            title: 'Plan review',
