...
import { SESSION_RESTORE_WINDOW_MS } from '@openpos/core';
...
describe('desktop session restore', () => {
    ...
    it('ignores unknown views and malformed payloads', () => {
        window.localStorage.setItem('openpos-last-view', JSON.stringify({ view: 'not-a-view', at: Date.now() }));
        expect(readRestorableLastView()).toBeNull();
        window.localStorage.setItem('openpos-last-view', 'not json');
        expect(readRestorableLastView()).toBeNull();
    });
});
