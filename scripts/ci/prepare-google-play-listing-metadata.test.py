...
def write_listing(
    ...
    *,
    title: str = "OpenPOS",
    short_description: str = "OpenPOS (mind water): local-first GTD capture.",
    full_description: str = 'OpenPOS (pronounced "mind water") keeps GTD tasks local.',
) -> None:
...
class PrepareGooglePlayListingMetadataTest(unittest.TestCase):
    def test_collects_localized_listing_payloads(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_listing(
                ...
                full_description="OpenPOS es un sistema GTD privado.",
            )
            write_listing(
                root,
                "fr-FR",
                short_description="Gestionnaire GTD privé.",
                full_description="OpenPOS est un gestionnaire GTD privé.",
            )
            write_listing(
                root,
                "de-DE",
                short_description="Privater GTD-Manager.",
                full_description="OpenPOS ist ein privater GTD-Manager.",
            )
            write_listing(
                root,
                "zh-CN",
                short_description="本地优先 GTD 任务管理器。",
                full_description="OpenPOS 是本地优先的 GTD 任务管理器。",
            )
        ...
        self.assertEqual(listings[0]["title"], "OpenPOS")
    ...
    def test_rejects_short_descriptions_over_google_play_limit(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            write_listing(root, "en-US", short_description="OpenPOS mind water " + ("x" * 80))
            write_listing(root, "de-DE")
            write_listing(root, "es-ES")
            write_listing(root, "fr-FR")
            write_listing(root, "zh-CN")
