...
try {
  await sendRequest("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "openpos-mcp-smoke", version: "0.0.0" },
  });
  initialized = true;
  sendNotification("notifications/initialized");
  const tools = await sendRequest("tools/list");
  if (!Array.isArray(tools?.tools) || !tools.tools.some((tool) => tool.name === "openpos_list_tasks")) {
    throw new Error("openpos_list_tasks was not advertised");
  }
  const result = await sendRequest("tools/call", {
    name: "openpos_list_tasks",
    arguments: { limit: 5 },
  });
  const text = result?.content?.[0]?.text;
  const payload = JSON.parse(text);
  if (!Array.isArray(payload.tasks)) {
    throw new Error("openpos_list_tasks did not return a tasks array");
  }
