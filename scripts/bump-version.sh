#!/usr/bin/env bash
# Version bump script for OpenPOS monorepo.
# Usage:
...
content = content.replace(
  /^(\s*source:\s*).*/m,
  `$1apps/desktop/src-tauri/target/release/bundle/deb/openpos_${version}_amd64.deb`
);
...
if ! node scripts/ci/check-mobile-foss-lock-sync.js; then
    ...
    echo "Repair it before tagging with:"
    echo "  cp apps/mobile/package.json /tmp/openpos-mobile-package.json.bak"
    ...
    echo "  cp /tmp/openpos-mobile-package.json.bak apps/mobile/package.json"
    exit 1
fi
